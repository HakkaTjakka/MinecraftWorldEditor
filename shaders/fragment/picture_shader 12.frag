...
vec3 render( in vec3 ro, in vec3 rd )
{
    ...
    if( res.y > -0.5 )
    {
        ...
        vec3 ff = texcube( texture_buffer, 0.1*vec3(pos.x,4.0*res.z-pos.y,pos.z), nor ).xyz;
        col *= ff.x;
...
//    float time = 5.0 + 0.2*iTime + 20.0*iMouse.x/iResolution.x;
    ...
    for( int i=0; i<ANTIALIAS; i++ )
    {
        ...
        vec2 off = vec2(0.0);
    #endif
        vec2 xy = (-iResolution.xy+2.0*((gl_FragCoord.xy-pos_correction.xy)+off)) / iResolution.y;
