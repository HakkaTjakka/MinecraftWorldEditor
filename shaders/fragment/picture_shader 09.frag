...
float noise( in float x ){return textureLod(texture_in, vec2(x*.01,1.),0.0).x;}
...
float noise(in vec3 p)
{
	...
	vec2 rg = textureLod( texture_in, (uv+ 0.5)/256.0, 0.0 ).yx;
...
void main()
{
	vec2 p = (gl_FragCoord.xy-pos_correction.xy)/iResolution.xy-0.5;
...
*/
 	trans=max(col.r,col.g);
  	trans=max(trans,col.b);
    gl_FragColor = vec4(col*1.3, trans);
}
