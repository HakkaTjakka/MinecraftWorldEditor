...
//	vec2 uv = gl_FragCoord.xy / iResolution.y * resolution;
	vec2 uv2 = vec2(1.0,1.0)*(gl_FragCoord.xy-pos_correction.xy) / txt_resolution.xy * resolution2;
	uv2.x=-uv2.x;
    ...
    for (int j = -MAX_RADIUS; j <= MAX_RADIUS; ++j)
    {
        for (int i = -MAX_RADIUS; i <= MAX_RADIUS; ++i)
        {
            ...
            float t = fract(0.1*iTime + hash12(hsh));
            vec2 v = p - uv;
    ...
    float intensity = mix(0.01, 0.15, smoothstep(0.1, 0.6, abs(fract(0.02*iTime + 0.5)*2.-1.)));
