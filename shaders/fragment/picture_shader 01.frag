...
//	d =  sdSphere(disp(p,1.0)*0.2 + p+   vec3(0.0,0.0,0.0),2.0) ;
...
/*
    for (int x=0; x<5; x++) {
        ...
        if ((where<PI*0.5 || (where>PI && where<1.5*PI)) ) {
                float d2 = ( 1.0-(0.5+sin(2.0*where+.5*PI)/2.0) ) +sdSphere(p +  ( rotateX(ff+T*0.33) * rotateY(ff+T*0.45) * rotateZ(ff+T*0.56) * vec4( 0.0+sin(where)*1.0, 0.0+sin(where)*1.0,  0.0+sin(where)*6.0  ,  1.0  )).xyz    ,
...
void main() {
    delta_buffer=1.0/iResolution.xy;
    T = iTime/4.0;
...
//    vec2 light_xy=rotate(iRotate)*vec2(-cos(TT)*15.7+1.0, 5.0);
    vec2 light_xy=rotate(iRotate)*vec2(-1.0, -1.0);
//	vec3 light=vec3(light_xy.x,light_xy.y, -2.5+cos(TT)*1.0+camera_pos); // zzzzz 2.0


//	vec3 light=vec3(-light_xy.x*(-(pos_abs.x)*7), -light_xy.y*((pos_abs.y)*7), 4.5+camera_pos); // zzzzz 2.0

//	vec3 light2=vec3(-light_xy.x*(-(pos_abs.x+0.4)*10), -light_xy.y*((pos_abs.y-0.4)*10), 4.5+camera_pos); // zzzzz 2.0
...
//	uv = zoom*rotate(iRotate)*(Coord.xy - .5 * Resolution) / Resolution.xy;
	vec3 rd = vec3(uv, 1.0);
...
//    uv2= vec2(1.0,-1.0)*  (( (   ( p.xy*1.2*vec2(1.0,-1.0) +  vec2(ppx,ppy) - t/1.5  ) ) -0.5) + 0.25);
	if (horizon==0 || m<0.0001) {// my raymarch function must be messed?
        ...
        vec2 offset=vec2(1.6,-3.0);       //yz

        vec3 n = normal( p );
        vec2 calc=(p.xy*vec2(0.6,0.6)*(1.1+p.z/1.50) +  vec2(ppx,ppy)  - (t)/2.5);
//        vec2 calc=(p.xy*vec2(0.8,0.6)*(1.1+p.z/1.50) +  vec2(ppx,ppy)  - (t)/2.5);
//        vec2 uv2= vec2(1.1,-1.1) * ( calc /(1.0+ p.xy/10.0) ) ;
//        vec2 uv2= vec2(1.1,-1.1) * ( calc /(1.0+ p.xy/10.0) )  + offset;
//        vec2 uv2= vec2(1.1,-1.1) * ( calc /(1.0+ p.xy/10.0) )  +  vec2(0.0,-(calc.y)*0.20)+ offset;
//        vec2 uv_adj=(uv2/(1.0+sqrt(-camera_pos/2.0 + 1.0)/iResolution)*vec2(0.14,0.14)  )*vec2(.3*iResolution.y/iResolution.x,.28)+ vec2(-0.4,1.32);
//        vec2 uv_adj=(uv2/(1.0+sqrt(-camera_pos/2.0 + 1.0)/iResolution)*vec2(0.126,0.165)  )*vec2(.3*iResolution.y/iResolution.x,.28)+ vec2(-0.4,1.32);

        vec2 uv_adj=(p.xy)/vec2(-4.0*iResolution.x/iResolution.y,4.0)*sqrt(iScale/2.0)+vec2(0.5,0.5);

        vec3 n2 = vec3(getGrad(uv_adj,-1.0/iResolution_buffer.xy),-150.0);
        n2=normalize(n2)*vec3(1.0,1.0,1.0);

        vec4 kD = texture(texture_in,uv_adj  );
//        vec4 kD = texture(texture_buffer,uv_adj  );

//        n=n-(n-n2)/1.0;

        vec3 lp = normalize(light - p);
        float diff = 1.2 * clamp(dot( n,lp), 0.3, 1.0);
        float spec = (1.5) * pow(max(dot(reflect(-lp, n), ro), 0.), 120.);

//        vec3 lp2 = normalize(light2 - p);
//        float diff2 = 1.0 * clamp(dot( n,lp2), 0.0, 1.);
//        float spec2 = (2.5) * pow(max(dot(reflect(-lp2, n), ro), 0.), 120.);

//		Color = clamp(vec4(kA + kD.xyz * (diff ) + kS *( spec ), 1.0), 0.0,1.0);
		Color = clamp(1.4*vec4(kA + kD.xyz * (diff) + kS * ( spec ), 1.0), 0.0,1.0);
//		Color = clamp(vec4(kA + kD.xyz * (diff ) , 1.0), 0.0,1.0);
//		Color = clamp(vec4(kA + kD.xyz , 1.0), 0.0,1.0);
//		Color = clamp(vec4(kA + kD.xyz * (diff ) + kS * ( spec ), 1.0), 0.0,1.0);
	} else {
		Color = vec4(0.0);
