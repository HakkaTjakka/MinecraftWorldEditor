...
{
    return vec3(0.0);
//    return texture(iChannel3, rd.yz).xyz;
}
...
vec3 shadeSupport(in vec3 pos, in vec3 ray)
{
    ...
    vec3 color3D = triPlanar(texture_in, pos, norm);
...
vec3 shadeTable(in vec3 pos, in vec3 ray)
{
    ...
    vec3 tex = triPlanar(texture_buffer, pos, nor);
    vec3 col = tex;
    ...
    else
    {
        ...
        col = triPlanar(texture_in, pos, nor);
...
vec3 shadeBall(in vec3 pos, in vec3 ray)
{
    ...
    if (ttr > 0.0)
        ...
        refl = vec3(0.0);
//        refl = texture(iChannel3, reflRay.xz).xyz;
...
void camPolar( out vec3 pos, out vec3 dir, vec3 origin, vec2 rotation, float dist, float zoom, vec2 offset, vec2 g )
//void camPolar( out vec3 pos, out vec3 dir, in vec3 origin, in vec2 rotation, in float dist, in float zoom, in vec2 offset, in vec2 g )
{
    ...
    // ray in view space
    dir.xy = (gl_FragCoord.xy-pos_correction.xy) - iResolution.xy*.5 + offset;
...
void main(  )
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        vec3 ro, rd;    
#if AA
        camPolar(ro, rd, camPos, camRot, 1.8 + 2.0 * iMouse.y * 0.002, 1.0, off[i], (gl_FragCoord.xy-pos_correction.xy);
#else
        camPolar(ro, rd, camPos, camRot, 1.8 + 2.0 * iMouse.y * 0.002, 1.0, vec2(0.0),(gl_FragCoord.xy-pos_correction.xy));
#endif
        ...
        if ((tBall > 0.0) && (tBall < tSupport || tSupport < 0.0) && (tBall < tTable || tTable < 0.0))
        {
			...
			trans=(renderColor.r+renderColor.g+renderColor.b)/3.0;
        }
