...
/*
    for (int x=0; x<10; x++) {
        float ff=float(x);
        d = smin(d, sdSphere(p + (  rotje * vec4( sin(T*0.354*ff)*1.0, sin(T*0.452*ff)*1.0,sin(T*0.399*ff)*1.0  ,  1.0)).xyz    ,
...
*/
	...
	d =  sdSphere(p+   vec3(0.0,0.0,0.0),1.0) ;

    for (int x=0; x<20; x++) {
        float ff=float(x);
        float where=mod(T*1.22+ff*ff,2*PI);
        if ((where<PI*0.5 || (where>PI && where<1.5*PI)) ) {
                float d2 =  ( 1.0 - (0.5+sin(2.0*where+.5*PI)/2.0) )*1.05
                            +sdSphere(p +  ( rotateX(ff+T*0.33) * rotateY(ff+T*0.45) * rotateZ(ff+T*0.56) * vec4( 0.0+sin(where)*0.5, 0.0+sin(where)*0.5,  0.0+sin(where)*6.0  ,  0.5  )).xyz    ,
                                    0.66*( 1.0-(0.5+sin(2.0*where+1.5*PI)/2.0) ));
              d = smin(d,d2,0.3);
        }
    }
...
// the blob
    ...
    for (int x=0; x<5; x++) {
        ...
        float where=mod(T*1.22+ff*ff,2*PI);
        if ((where<PI*0.5 || (where>PI && where<1.5*PI)) ) {
            float d2 = ( 1.0-(0.5+sin(2.0*where+.5*PI)/2.0) )*disp(p,1.0) +sdSphere(p +  ( rotateX(ff+T*0.33) * rotateY(ff+T*0.45) * rotateZ(ff+T*0.56) * vec4( 0.0+sin(where)*2.0, 0.0+sin(where)*2.0,  0.0+sin(where)*4.0  ,  1.0  )).xyz    ,   1.33*( 1.0-(0.5+sin(2.0*where+1.5*PI)/2.0) ));
            d = smin(d,d2,0.3);
        }
    }
*/
//	d =  disp(p,1.0)+sdSphere(p+   vec3(0.0,0.0,0.0),2.0) ;

//sphere only
//	d =  sdSphere(p+   vec3(0.0,0.0,0.0),2.0) ;
...
void main() {
    ...
    T = iTime/4.0;
    Resolution=iResolution*vec2(1.0,1.0);
    back=0;
    vec4 Color=vec4(0.0);
...
//	vec3 light=vec3(-cos(TT)*15.7+1.0, 5., -2.5+cos(TT)*1.0+camera_pos); // zzzzz 2.0
    ...
    vec2 light_abs=sin(pos_abs*2*PI)/2.0;

	vec3 light=vec3(-light_abs.x*30, light_abs.y*30, 5.5+camera_pos); // zzzzz 2.0
	vec3 light2=vec3(-light_abs.x*20+20, light_abs.y*20-20, 3.5+camera_pos); // zzzzz 2.0

//	vec3 light=vec3(-pos_abs.x*30, pos_abs.y*30, 5.5+camera_pos); // zzzzz 2.0
//	vec3 light2=vec3(-pos_abs.x*20+20, pos_abs.y*20-20, 3.5+camera_pos); // zzzzz 2.0
...
//        vec3 prot=(rotje*vec4(p,1.0)).xyz;
        vec3 n = normal( p );
        vec2 calc=(p.xy*vec2(0.6,0.5)*(1.1+p.z/1.50) +  vec2(ppx,ppy)  - (t)/2.5);
//        vec2 uv2= vec2(1.1,1.1) * ( calc /(1.0+ p.xy/10.0) )  +  vec2(0.0,-(calc.y)*0.20);
...
//            vec2 uv_adj=(uv2/(1.0+sqrt(-camera_pos + 1.0)/iResolution_buffer.y)*vec2(0.05,0.05) * rotate(T*0.03)+ vec2(sin(T*0.042),cos(T*0.033)));
        ...
        vec2 uv_adj=rotate(iRotate)*(uv2/(1.0+sqrt(-camera_pos/2.0 + 1.0)/iResolution_buffer.y)*vec2(0.08,0.08)  )+offset;
...
//    float diff=clamp(dot(n,light),0.5,1.0);
        ...
        n=normalize(n); //no lightning on paint
...
//		Color = clamp(vec4(kA + kD.xyz * (diff ) , 1.0), 0.0,1.0);
		Color = clamp(vec4(kA + kD.xyz * (diff+diff2 ) + kS * ( spec+spec2 ), 1.0), 0.0,1.0);
	} else {
		Color = vec4(0.0);
