...
void main(){
    ...
    // Screen coordinates.
	vec2 u = ((gl_FragCoord.xy-pos_correction.xy) - iResolution.xy*.5) / iResolution.y;


	// Camera Setup.
//	vec3 lk = vec3(-pos_correction.x/500, 0, -pos_correction.y/500);  // "Look At" position.
	vec3 lk = vec3(0, 0, iTime*0.2);  // "Look At" position.
    ...
    if(t<FAR){
        ...
        // texture a bit.
        float txSc = .5;
        vec3 tx = tex3D(texture_in, (p*txSc), n);
    ...
    // Vignette.
    u = (gl_FragCoord.xy-pos_correction.xy)/iResolution.xy;
