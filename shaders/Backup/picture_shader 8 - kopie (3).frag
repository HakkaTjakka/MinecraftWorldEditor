...
/*
  vec3 cylinder = (rotje * vec4(p.x,p.y,p.z, 1.0)).xyz;
	d = sdCappedCylinder(cylinder,vec2(.50,2.0)) - 0.3 ;

    cylinder = (rotateZ(0.5*PI) * rotje * vec4(p.x,p.y,p.z, 1.0)).xyz;
	float d2 = sdCappedCylinder(cylinder,vec2(.50,2.0)) - 0.3 ;

    d = smin( d, d2, .2);

    cylinder = (rotateZ(0.5*PI) * rotateY(0.5*PI) * rotje * vec4(p.x,p.y,p.z, 1.0)).xyz;
...
//    d2 = 1.0E10;
//    for (int x=0; x<8; x++) {
//        float ff=float(x)/8.0;
//        d2 = smin(d2, sdSphere(p + (  rotje2 * vec4( 0.0, 0.0, sin(T*0.343 + 2.0*PI*ff)*3.5  ,  1.0)).xyz    ,
//                             0.5 ) // size sphere
//                  ,.2)   ;
//    }
//    d = min(d,d2);

	d =  min ( sdSphere(p+vec3(0.0,0.0,0.0),2.0), d );
...
//	float ppx = .0+cos(T*.4+sin(T*0.5)*3.14)*.1;
	...
	vec3 light=vec3(-cos(TT)*1.7+3.14/4.0, 1., -2.0+3.14*-0.5+0.2*cos(TT)*1.4+camera_pos); // zzzzz 2.0
...
//        vec2 offset=vec2(-T*0.1*1.0,-T*0.1*1.0);  //xyz
...
//        vec2 uv2= vec2(.5,.5) * ( calc   )  +  vec2(0.0,-(calc.y)*0.20);
//        vec2 uv_adj;
...
//        uv_adj=(uv2/(1.0+sqrt(-camera_pos + 1.0)/iResolution_buffer.y)*vec2(0.05,0.05)  );
...
//        vec2 uv_adj=vec2(mod(angles.y,2.0*PI)/2.0*PI, (PI-angles.x)/PI );
        ...
        vec3 norm=(rotje*vec4(n,1.0)).xyz;
...
//        kD=vec4( kD.x+sin( (p.x+p.z)/3.0+T*0.24)/4.0, kD.y+sin((p.y+p.x)/3.0+T*0.35)/4.0, kD.z+sin((p.z+p.y)/3.0+T*0.29)/4.0,1.0);
    ...
    vec3 n2 = vec3(getGrad(uv_adj,1.0/iResolution_buffer.xy),150.0);
        ...
        float diff2 = 1.75 * clamp(dot(lp2, n2), 0.2, 1.);
        float spec2 = (9.5) * pow(max(dot(reflect(-lp2, n2), ro), 0.), 120.);

        vec3 lp = normalize(light - p);
        float diff = 1.75 * clamp(dot(lp, n), 0.2, 1.);
        float spec = (9.5) * pow(max(dot(reflect(-lp, n), ro), 0.), 120.);

//		Color = clamp(vec4(kA + kD.xyz * (diff ) + kS *( spec ), 1.0), 0.0,1.0);
		Color = clamp(vec4(kA + kD.xyz * (diff + diff2/1.0) + kS *( spec + spec2/10.0), 1.0), 0.0,1.0);
	} else {
		Color = vec4(0.0);
