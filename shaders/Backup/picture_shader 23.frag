...
float ruis2( vec2 x )
{
    ...
    return float(mix(s1.x,s1.y,f.y)) ;
}
...
float ruis3( vec3 x )
{
    ...
    return float(mix(mix(s1.x,s1.y,f.y),mix(s1.z,s1.w,f.y),f.z));
}
...
vec4 booble(vec2 te,vec2 pos,float numCells)
{
    ...
    vec2 te2=-te1;
    float zb1=max(pow(ruis2(te2*1000.11*d),10.0),0.01);
    float zb2=ruis2(te1*1000.11*d);
    float zb3=ruis2(te1*200.11*d);
    float zb4=ruis2(te1*200.11*d+vec2(20.0));

    vec4 colorb=vec4(1.0);
    colorb.xyz=colorb.xyz*(0.7+ruis2(te1*1000.11*d)*0.3);

    zb2=max(pow(zb2,20.1),0.01);
    colorb.xyz=colorb.xyz*(zb2*1.9);

    vec4 color=vec4(ruis2(te2*10.8),ruis2(te2*9.5+vec2(15.0,15.0)),ruis2(te2*11.2+vec2(12.0,12.0)),1.0);
    color=mix(color,vec4(1.0),ruis2(te2*20.5+vec2(200.0,200.0)));
    color.xyz=color.xyz*(0.7+ruis2(te2*1000.11*d)*0.3);
...
vec4 Cells(vec2 p,vec2 move, in float numCells,in float count,float blur)
{
	...
	for (int xo = -1; xo <= 1; xo++)
	{
		for (int yo = -1; yo <= 1; yo++)
		{
            ...
            tp=tp + (hash2a(rr,nulTime*0.1)+hash2a(rr,nulTime*0.1+0.25))*0.5;
			vec2 l = inp - tp;
            float dr=dot(l, l);
    ...
    //te=te+(te*ruis3(vec3(te*5.9,nulTime*40.0))*0.02);
    //te=te+(te*(ruis3(vec3(te*3.9+p,nulTime*0.2))+ruis3(vec3(te*3.9+p,nulTime*0.2+0.25))+ruis3(vec3(te*3.9+p,nulTime*0.2+0.5))+ruis3(vec3(te*3.9+p,nulTime*0.2+0.75)))*0.05);
...
void main()
{
	uv = (gl_FragCoord.xy-pos_correction.xy) / iResolution.y*10.0;

    vec2 l1=vec2(nulTime*0.02,nulTime*0.02);
    vec2 l2=vec2(-nulTime*0.01,nulTime*0.007);
    vec2 l3=vec2(0.0,nulTime*0.01);

    //vec4 e=vec4(ruis2(uv*2.0),ruis2(uv*2.0+vec2(200.0)),ruis2(uv*2.0+vec2(50.0)),0.0);
    vec4 e=vec4(ruis3(vec3(uv*2.0,nulTime*0.1)),ruis3(vec3(uv*2.0+vec2(200.0),nulTime*0.1)),ruis3(vec3(uv*2.0+vec2(50.0),nulTime*0.1)),0.0);
    ...
    //e=e*(3.0+sin(nulTime*10.0)*0.4+sin(nulTime*5.0)*0.5+sin(nulTime*100.0)*0.05 )*0.25;
