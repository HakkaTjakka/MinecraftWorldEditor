...
{
    return length(texture(texture_in,uv).xyz);
}
...
void main( )
{
	vec2 uv = (gl_FragCoord.xy-pos_correction.xy) / iResolution.xy*vec2(1,-1);
...
//	gl_FragColor = texture(texture_in,uv)*vec4(vec3(diff),1.0)+vec4(vec3(spec),0.0);
