...
vec4 AA_one(vec2 uv, vec2 delta)
{
    vec4 col=vec4(0.0);
    col+=texture(texture_in,uv)*4.0;
    col+=texture(texture_in,uv+vec2(delta.x,0.0)        )*2.0;
    col+=texture(texture_in,uv+vec2(-delta.x,0.0)       )*2.0;
    col+=texture(texture_in,uv+vec2(0.0,delta.y)        )*2.0;
    col+=texture(texture_in,uv+vec2(0.0,-delta.y)       )*2.0;
    col+=texture(texture_in,uv+vec2(delta.x,delta.y)    );
    col+=texture(texture_in,uv+vec2(-delta.x,delta.y)   );
    col+=texture(texture_in,uv+vec2(delta.x,-delta.y)   );
    col+=texture(texture_in,uv+vec2(-delta.x,-delta.y)  );
    delta*=2.0;
    col+=texture(texture_in,uv+vec2(delta.x,0.0)        );
    col+=texture(texture_in,uv+vec2(-delta.x,0.0)       );
    col+=texture(texture_in,uv+vec2(0.0,delta.y)        );
    col+=texture(texture_in,uv+vec2(0.0,-delta.y)       );
...
float getVal_super_multi(vec2 uv, vec2 delta)
{
...
//void main()
{
    ...
    vec3 light=vec3(1.0, 1.0, -5.0); // zzzzz 2.0
    ...
    if (formula==2 && background_plot==1 ) {         //dont use....formula=2 on this one.
        uv = (gl_FragCoord.xy) / vec2(1920,1080);
...
void main_aa()
{
    if (background_plot==1) {
        ...
        vec2 uv = vec2(1.0,-1.0)*(gl_FragCoord.xy-pos_correction.xy) / iResolution.xy;
        gl_FragColor = AA_one(uv,1.0/iResolution_buffer.xy); // iResolution_buffer = size picture. iResolution = size picture * scale . We need picture pixels, not screen pixels.
    }
}
