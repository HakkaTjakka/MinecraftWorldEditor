...
const vec3 skycolour2 = vec3(0.4, 0.4, 1.0);
...
//	vec2 uv = (gl_FragCoord.xy-vec2(-1.0,1.0)*txt_correction.xy) / txt_resolution.xy * resolution;
//  uv.x=-uv.x;

//    vec2 p = (gl_FragCoord.xy-pos_correction.xy) / iResolution.xy;
//    vec2 p2 = (gl_FragCoord.xy-txt_correction.xy) / iResolution.xy;
    vec2 p2 = (gl_FragCoord.xy-vec2(-1.0,1.0)*txt_correction.xy) / iResolution.xy;
...
//    float q = fbm(uv * cloudscale * 0.5);
    ...
    float weight = 0.8;
    for (int i=0; i<18; i++){
		r += abs(weight*noise( uv2+iTime/100.0 ) );
        uv2 = m*uv2 + time;
		weight *= 0.7;
    }
    ...
    weight = 0.7;
    for (int i=0; i<8; i++){
		f += weight*noise( uv2+iTime/100.0 );
        uv2 = m*uv2 + time;
		weight *= 0.6;
    }
