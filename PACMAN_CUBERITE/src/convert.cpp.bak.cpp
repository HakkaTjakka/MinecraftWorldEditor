...
//points in filename without extensions.
        sprintf(convert_path,"%s/list.txt",CONVERTDIR);
        ret=select_from_list2(link,convert_path,line_count);
//        ret=select_from_list2(link,"../convert/list.txt",line_count);
        if (ret==0) {
            printf("%s EMPTY? CONVERTDIR in pacman.ini set?\n",convert_path);
...
//                        monitor_off=1;
                        ...
                        int audio_ok=0;
                        int subs_ok=0;
//ADAPTED FOR . IN BASEFILENAME
                        sprintf(test,"%s/files/%s.f%d.%s",CONVERTDIR,filename,videonum,container_video.extension);
...
//                            sprintf(videoname,"%s.f%d.webm",filename,videonum);
                            ...
                            char check[1000]="";
                            sprintf(check,"%s/files/%s.f%d.%s.part",CONVERTDIR,filename,videonum,container_video.extension);
//                            sprintf(check,"../convert/files/%s.f%d.%s.part",filename,videonum,container_video.extension);
    ...
    //                            sprintf(videoname,"%s.f%d.webm",filename,videonum);
                                ...
                                printf("TESTED VIDEO: NO MATCH %s/files/%s.f%d.%s\n",CONVERTDIR,filename,videonum,container_video.extension);
//                                printf("TESTED VIDEO: NO MATCH ../convert/files/%s.f%d.%s\n",filename,videonum,container_video.extension);
                        }
                        sprintf(test,"%s/files/%s.f%d.%s",CONVERTDIR,filename,audionum,container_audio.extension);
//                        sprintf(test,"../convert/files/%s.f%d.%s",filename,audionum,container_audio.extension);
                        if (file_exists(test)) {
                            ...
                            printf("TESTED AUDIO: NO MATCH  %s/files/%s.f%d.%s\n",CONVERTDIR,filename,audionum,container_audio.extension);
...
//                        sprintf(test,"../convert/files/%s.%s",filename,SUBTITLES);
                        sprintf(test,"%s/files/%s.%s.%s",CONVERTDIR,filename,SUBTITLES,SUBTITLE_FORMAT);
...
//                            sprintf(subsname,"%s.%s.vtt",filename,SUBTITLES);
                            ...
                            strcpy(lang,SUBTITLES);
                                ...
                                printf("TESTED SUBTITLES: NO MATCH %s/files/%s.%s.%s\n",CONVERTDIR,filename,SUBTITLES,SUBTITLE_FORMAT);
...
//                                printf("TESTED SUBTITLES: NO MATCH ../convert/files/%s.%s.vtt\n",filename,SUBTITLES);
                                sprintf(test,"%s/files/%s.%s.%s",CONVERTDIR,filename,"en",SUBTITLE_FORMAT);
...
//                                    sprintf(subsname,"%s.%s.vtt",filename,"en");
                                    ...
                                    printf("TESTED SUBTITLES: NO MATCH %s/files/%s.%s.%s\n",CONVERTDIR,filename,"en",SUBTITLE_FORMAT);
//                                    printf("TESTED SUBTITLES: NO MATCH ../convert/files/%s.%s.%s\n",filename,"en",SUBTITLE_FORMAT);
//                                    printf("TESTED SUBTITLES: NO MATCH ../convert/files/%s.%s.vtt\n",filename,"en");
                                }
                        }
                        if (subs_ok==1 && strcmp("srt",SUBTITLE_FORMAT)==0) {
                            sprintf(com,"subtitle-overlap-fixer %s/files/\"%s\"",CONVERTDIR,subsname);
//                            sprintf(com,"subtitle-overlap-fixer ../convert/files/\"%s\"",subsname);
                        ...
                        if (video_ok && audio_ok && subs_ok) {
                            ...
                            textfiles.lock();
                            sprintf(convert_path,"%s/files/convert.txt",CONVERTDIR);
                            if (file=fopen(convert_path,"a")) {
//                            if (file=fopen("../convert/files/convert.txt","a")) {
                                ...
                                printf("CAN NOT WRITE TO FILE %s/files/convert.txt\n",CONVERTDIR);
//                                printf("CAN NOT WRITE TO FILE ../convert/files/convert.txt\n");
                            }
                            textfiles.unlock();
                            sprintf(convert_path,"%s/files/convert.bat",CONVERTDIR);
                            if (file=fopen(convert_path,"a")) {
...
//                                    go_burn=0;
                                ...
                                printf("CAN NOT WRITE TO FILE %s/files/convert.bat\n",CONVERTDIR);
//                                printf("CAN NOT WRITE TO FILE ../convert/files/convert.bat\n");
                        ...
                        if (video_ok && audio_ok) {
                            textfiles.lock();
                            sprintf(convert_path,"%s/files/list.txt",CONVERTDIR);
                            if (file=fopen(convert_path,"a")) {
//                            if (file=fopen("../convert/files/list.txt","a")) {
                                if (videonum==audionum) {
                                    ...
                                    sprintf(test,"%s/files/%s.%s",CONVERTDIR,filename,container_video.extension);
//                                    sprintf(test,"../convert/files/%s.%s",filename,container_video.extension);
                                    if (file_exists(test))
                                        fprintf(file,"%s.%s\n",filename,container_video.extension);
                                    else {
                                        sprintf(test,"%s/files/%s.mkv",CONVERTDIR,filename);
//                                        sprintf(test,"../convert/files/%s.mkv",filename);
                                        if (file_exists(test))
                                            fprintf(file,"%s.mkv\n",filename);
                                        else {
                                            sprintf(test,"%s/files/%s.mp4",CONVERTDIR,filename);
//                                            sprintf(test,"../convert/files/%s.mp4",filename);
                                            if (file_exists(test))
                                                fprintf(file,"%s.mp4\n",filename);
                                            else {
                                                sprintf(test,"%s/files/%s.avi",CONVERTDIR,filename);
//                                                sprintf(test,"../convert/files/%s.avi",filename);
                                                if (file_exists(test))
                                                    fprintf(file,"%s.avi\n",filename);
                                                else {
                                                    sprintf(test,"%s/files/%s.webm",CONVERTDIR,filename);
//                                                    sprintf(test,"../convert/files/%s.webm",filename);
                                ...
                                printf("CAN NOT WRITE TO FILE %s/files/list.txt\n",CONVERTDIR);
//                                printf("CAN NOT WRITE TO FILE ../convert/files/list.txt\n");
                            }
                            sprintf(convert_path,"%s/converted/list.txt",CONVERTDIR);
                            if (file=fopen(convert_path,"a")) {
...
//                                                    sprintf(test,"../convert/files/%s.webm",filename);
                                ...
                                printf("CAN NOT WRITE TO FILE %s/converted/list.txt\n",CONVERTDIR);
...
void burn_next_sub() {
    ...
    textfiles.lock();
    sprintf(convert_path,"%s/files/convert.txt",CONVERTDIR);
    ret=select_from_list2(data,convert_path,line_count);
...
//        ret=select_from_list2(data,"../convert/files/convert.txt",line_count);
        ...
        if (ret==0) {
            sprintf(print_line,"%s EMPTY?",convert_path);
            printf("%s\n",print_line);
            line_count=0;
            burn_next_sub_thread_running=0;
            return;
...
//multiple points....   videoname.fxyz.mp4 -> videoname
            strcpy(data,videoname);
            while (strlen(data)>=1 && data[strlen(data)-1]!='.') {
                data[strlen(data)-1]='\0';
            }
...
//            sscanf(videoname,"%999[^.]",data);
            ...
            sprintf(videoname_nq,           "%s/files/%s",CONVERTDIR,videoname);
            sprintf(audioname_nq,           "%s/files/%s",CONVERTDIR,audioname);
            sprintf(subsname_nq,            "%s/files/%s",CONVERTDIR,subsname);
...
*/
                ...
                if (file_exists(videoname_nq) || file_exists(audioname_nq) || file_exists(subsname_nq) ) {
                    disp_open();
                        sprintf(print_line,"MOVING FILES FROM %s/FILES/ TO %s/CONVERTED/ ...",CONVERTDIR,CONVERTDIR);
//                        sprintf(print_line,"MOVING FILES FROM ../CONVERT/FILES/ TO ../CONVERT/CONVERTED/ ...");
                    ...
                    sprintf(convert_path,"move \"%s/files/%s*.*\" %s/converted/",CONVERTDIR,raw_name,CONVERTDIR);
                    while (replace_str(convert_path,"/","\\"));

//here
                    sprintf(str_part,"%s",convert_path);
//                    sprintf(str_part,"move \"..\\convert\\files\\%s*.*\" ..\\convert\\converted\\",raw_name);
    ...
    //                sprintf( print_line, "VIDEO    = \"%s\"%s NOT FOUND!!!",videoname_nq,video_add);
                    sprintf( print_line, "VIDEO    = \"%s\"%s (%s/files) NOT FOUND!!!",videoname, video_add,CONVERTDIR );
//                    sprintf( print_line, "VIDEO    = \"%s\"%s (../convert/files) NOT FOUND!!!",videoname, video_add );
                ...
                disp_open();
                    ...
                    sprintf( print_line, "VIDEO    = \"%s\"%s (%s/files) %s",videoname, video_add,CONVERTDIR, get_info_video(videoname_nq));
//                    sprintf( print_line, "VIDEO    = \"%s\"%s (../convert/files) %s",videoname, video_add, get_info_video(videoname_nq));
            ...
            if (!file_exists(audioname_nq)) {
                ...
                printf("%s\n",print_line);
                cont=1;
                sprintf(print_line,"AUDIO    = \"%s\"%s (%s/files) NOT FOUND!!!",audioname_nq,audio_add,CONVERTDIR);
//                sprintf(print_line,"AUDIO    = \"%s\"%s (../convert/files) NOT FOUND!!!",audioname_nq,audio_add);
                ...
                sprintf(print_line,"AUDIO    = \"%s\"%s (%s/files) %s",audioname, audio_add,CONVERTDIR, get_info_audio(audioname_nq));
//                sprintf(print_line,"AUDIO    = \"%s\"%s (../convert/files) %s",audioname, audio_add, get_info_audio(audioname_nq));
            ...
            if (!file_exists(subsname_nq)) {
                ...
                printf("%s\n",print_line);
                cont=1;
                sprintf(print_line,"SUBTITLE = \"%s\"%s (%s/files) NOT FOUND!!!",subsname_nq,subs_add,CONVERTDIR);
//                sprintf(print_line,"SUBTITLE = \"%s\"%s (../convert/files) NOT FOUND!!!",subsname_nq,subs_add);
                print_to_screen_buf(1,print_line,PRINT_FONT_SIZE,COL_WHITE,COL_RED,COL_WHITE_T,PRINT_FONT_OUTLINE,TXT_REG);

                disp_close2();
            } else {
                sprintf(print_line,"SUBTITLE = \"%s\"%s (%s/files)",subsname,subs_add,CONVERTDIR);
//                sprintf(print_line,"SUBTITLE = \"%s\"%s (../convert/files)",subsname,subs_add);
            ...
            sprintf(quote_video,"\"%s/files/%s\"",CONVERTDIR,videoname);
            sprintf(quote_audio,"\"%s/files/%s\"",CONVERTDIR,audioname);
            sprintf(quote_subs, "\"%s/files/%s\"",CONVERTDIR,subsname);
...
//            while (replace_str(converted,"[dot_comma]","\\'\\;\\\\'"));
            ...
            sprintf(quote_subs_extra, "\"f='%s/files/%s'\"",CONVERTDIR,converted);
