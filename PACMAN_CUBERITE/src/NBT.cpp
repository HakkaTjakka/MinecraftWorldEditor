...
void toggle() {
    if (set_quiet) return;
    static int toggle=0;
    if (toggle++>=100000) toggle=0;
    if (toggle==0) printf("./\b");
    else if (toggle==24900) printf("-\b");
    else if (toggle==49900) printf("\\\b");
    else if (toggle==74900) printf("|\b");
}
...
*/
    ...
    switch (tag->type)
    {
        ...
        case NBT_TYPE_FLOAT_ARRAY: {
            fprintf(file,"[float array](%d) \"%s\"",tag->tag_float_array.size,tag->name);
            for (size_t i = 0; i < tag->tag_float_array.size; i++) fprintf(file," %f", tag->tag_float_array.value[i]); fprintf(file,"\r\n");
            break; }
        case NBT_TYPE_DOUBLE_ARRAY: {
            fprintf(file,"[double array](%d) \"%s\"",tag->tag_double_array.size,tag->name);
            for (size_t i = 0; i < tag->tag_double_array.size; i++) fprintf(file," %.*e", DECIMAL_DIG, tag->tag_double_array.value[i]); fprintf(file,"\r\n");
            break; }
...
//      break;
        ...
        {
            printf("%f ", tag->tag_float_array.value[i]);
        }
        break;
    }
    case NBT_TYPE_DOUBLE_ARRAY:
...
//      break;
        ...
        {
            printf("%f ", tag->tag_double_array.value[i]);
        }
        break;
    }
    default:
...
//            printf( "sizeof(*intern[(size_t) %lld])=%lld\n" , (size_t) userdata , sizeof( *intern[(size_t) userdata]) );
            ...
            intern[(size_t) userdata] = (uint8_t*)NBT_REALLOC( intern[(size_t) userdata], current_position[(size_t) userdata]  );
...
void WRITE_REGION_FROM_DIRS_NBT(char* regionname) {
    struct stat stat_buffer;
    char filename[1024];
    sprintf(filename,"%s.mca",regionname);
    nbt_tag_t* tag_region=nbt_new_tag_compound();
...
//        if (!set_quiet) printf("Reading GZIP file to mem: %s\n",filename);
        ...
        if (!set_quiet) printf("Reading GZIP file: %s ",filename);
//        int sqo=set_quiet;
//        set_quiet=1;
...
//        tag_region=read_nbt_file(filename, type);
    ...
    if (!set_quiet) { tied=GetCounterNanoseconds(); StartCounter(); tied_total=tied_total+tied; }
    if (!set_quiet) printf("\r%7.3f -%7.3f\n",tied_total,tied);

    if (!set_quiet) printf("%7.3f -%7.3f ",tied_total,0.0);
    if (!set_quiet) printf("Writing region file: %s ",tag_region->name);
    strcpy(world_nbt_filename,filename);
...
uint8_t* main_nbt2mca(nbt_tag_t* tag_region, size_t* size)
{
    ...
    if (!repack) { printf("                                          \r");printf(name); }
...
void MAKE_MCA_FROM_WORLD_NBT(char* worldname) {
    char dir[1024];
    char sourcedir[1024];
    char filename[1024];
    char regionname[1024];
        ...
        if ((strstr(de->d_name, "r.")) != NULL) {
            ...
            if( stat(filename,&s) == 0 ) {
                if( s.st_mode & S_IFDIR )  { // if( s.st_mode & S_IFREG  for file....
                    ...
                    for (int t=0; t<1024; t++)
                    {
                        sprintf(filename,"%s/%d.nbt",sourcedir,t);
                        if (stat(filename,&s)==0)
                        {
                            tag_index->tag_int_array.value[t]=chunk_count+1;
                            chunk_count++;
