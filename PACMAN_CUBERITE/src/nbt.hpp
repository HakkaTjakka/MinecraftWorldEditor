...
#ifdef DEBUG_MEMORY
    printf("1");
    fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:284) tag\n",tag,(size_t)sizeof(nbt_tag_t));         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
    printf("2");
    fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:309) tag->name=%s\n", tag->name, (size_t)tag->name_size + 1, tag->name);         //DEBUG_MEM
#endif
    }
    else
    {
...
#ifdef DEBUG_MEMORY
printf("3");
        fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:362) tag->tag_byte_array.value\n", tag->tag_byte_array.value, (size_t)tag->tag_byte_array.size );         //DEBUG_MEM
#endif
        break;
    }
...
#ifdef DEBUG_MEMORY
printf("4");
        fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:374) tag->tag_string.value\n", tag->tag_string.value, (size_t)tag->tag_string.size + 1 );         //DEBUG_MEM
#endif
        break;
    }
    case NBT_TYPE_LIST:
...
#ifdef DEBUG_MEMORY
printf("5");
        fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:383) tag->tag_list.value\n", tag->tag_list.value, (size_t)(tag->tag_list.size * sizeof(nbt_tag_t*)) );         //DEBUG_MEM
#endif
    ...
    case NBT_TYPE_COMPOUND:
    {
        ...
        for (;;)
        {
            ...
            {
//                fprintf(file_free,"%p(3)            nbt__parse(line:400) inner_tag\n", inner_tag);         //DEBUG_MEM
...
#ifdef DEBUG_MEMORY
printf("6");
                fprintf(file_realloc,"%p(2)            -> ", tag->tag_compound.value);         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("7");
                fprintf(file_realloc,"%p %10llu nbt__parse(line:408) tag->tag_compound.value(size=(+1=)%llu)\n", tag->tag_compound.value, (size_t)(tag->tag_compound.size + 1) * (size_t)sizeof(nbt_tag_t*), (size_t)tag->tag_compound.size +1 );         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("8");
        fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:421) tag->tag_int_array.value\n", tag->tag_int_array.value, (size_t)(tag->tag_int_array.size * sizeof(int32_t)) );         //DEBUG_MEM
#endif
        break;
    }
...
#ifdef DEBUG_MEMORY
printf("9");
        fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:432) tag->tag_long_array.value\n", tag->tag_long_array.value, (size_t)(tag->tag_long_array.size * sizeof(int64_t)) );         //DEBUG_MEM
#endif
        break;
    }
    case NBT_TYPE_FLOAT_ARRAY:
    {
        tag->tag_float_array.size = nbt__get_int32(stream);
        tag->tag_float_array.value = (float*)NBT_MALLOC(tag->tag_float_array.size * sizeof(float));
        for (size_t i = 0; i < tag->tag_float_array.size; i++)
        {
            tag->tag_float_array.value[i] = nbt__get_float(stream);
        }
#ifdef DEBUG_MEMORY
printf("10");
        fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:432) tag->tag_float_array.value\n", tag->tag_float_array.value, (size_t)(tag->tag_float_array.size * sizeof(float)) );         //DEBUG_MEM
#endif
        break;
    }
    case NBT_TYPE_DOUBLE_ARRAY:
    {
        tag->tag_double_array.size = nbt__get_int32(stream);
        tag->tag_double_array.value = (double*)NBT_MALLOC(tag->tag_double_array.size * sizeof(double));
        for (size_t i = 0; i < tag->tag_double_array.size; i++)
        {
            tag->tag_double_array.value[i] = nbt__get_double(stream);
        }
#ifdef DEBUG_MEMORY
printf("11");
        fprintf(file_malloc,"%p(1) %10llu nbt__parse(line:432) tag->tag_double_array.value\n", tag->tag_double_array.value, (size_t)(tag->tag_double_array.size * sizeof(double)) );         //DEBUG_MEM
#endif
        break;
    }
    default:
...
#ifdef DEBUG_MEMORY
printf("10");
        fprintf(file_free,"%p(3)        nbt__parse(line:437) tag\n", tag );         //DEBUG_MEM
#endif
        NBT_FREE(tag);
        printf("nbt__parse default????????????????????????????????????????????????????????????????????????????????????????????????????\n");
        return NULL;
    }
...
#ifdef DEBUG_MEMORY
printf("12");
            fprintf(file_free,"%p(3)            nbt__parse(line:538) buffer\n", buffer);         //DEBUG_MEM
#endif
            NBT_FREE(buffer);
            return NULL;
        }
        ...
        size_t buffer_size_real = 1024*1024;
        buffer = (uint8_t*)NBT_REALLOC(buffer, buffer_size_real);
        do
        {
...
#ifdef DEBUG_MEMORY
printf("13");
                fprintf(file_realloc,"%p(2)            -> ", buffer);         //DEBUG_MEM
#endif

                if (buffer_size_real<buffer_size+have) {
                    buffer_size_real *= 2;
                    buffer = (uint8_t*)NBT_REALLOC(buffer, buffer_size_real);
                }
...
#ifdef DEBUG_MEMORY
printf("14");
                fprintf(file_realloc,"%p %10llu nbt_parse (line:559) buffer\n",buffer, (size_t)( buffer_size + have) );         //DEBUG_MEM
#endif
    ...
    else
    {
        ...
        uint8_t in_buffer[NBT_BUFFER_SIZE];
        size_t bytes_read;
        size_t buffer_size_real=NBT_BUFFER_SIZE;
        buffer = (uint8_t*)NBT_REALLOC(buffer, buffer_size_real);
        do
        {
...
printf("15");
            fprintf(file_realloc,"%p(2)            -> ", buffer);         //DEBUG_MEM
#endif
            if (buffer_size+bytes_read>buffer_size_real) {

                buffer_size_real=buffer_size_real*2;
                buffer = (uint8_t*)NBT_REALLOC(buffer, buffer_size_real);
...
#ifdef DEBUG_MEMORY
printf("16");
            fprintf(file_realloc,"%p %10llu nbt_parse(line:582)  buffer\n", buffer,  (size_t)(buffer_size + bytes_read) );         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("17");
    fprintf(file_free,"%p(3)            nbt_parse(line:598)  buffer\n", buffer );         //DEBUG_MEM
#endif
    NBT_FREE(buffer);
...
#ifdef DEBUG_MEMORY
printf("18");
        fprintf(file_realloc,"%p(2)            -> ", stream->buffer);         //DEBUG_MEM
#endif
        stream->buffer = (uint8_t*)NBT_REALLOC(stream->buffer, stream->alloc_size * 2);
        stream->alloc_size *= 2;
#ifdef DEBUG_MEMORY
printf("19");
        fprintf(file_realloc,"%p %10llu put_byte (line:619)  stream->buffer\n", stream->buffer, (size_t)stream->alloc_size * 2 );         //DEBUG_MEM
#endif
    }
...
//            if (tag->tag_compound.value[i])
    ...
    case NBT_TYPE_FLOAT_ARRAY:
    {
        nbt__put_int32(stream, tag->tag_float_array.size);
        for (size_t i = 0; i < tag->tag_float_array.size; i++)
        {
            nbt__put_float(stream, tag->tag_float_array.value[i]);
        }
        break;
    }
    case NBT_TYPE_DOUBLE_ARRAY:
    {
        nbt__put_int32(stream, tag->tag_double_array.size);
        for (size_t i = 0; i < tag->tag_double_array.size; i++)
        {
            nbt__put_double(stream, tag->tag_double_array.value[i]);
        }
        break;
    }
    default:
...
#ifdef DEBUG_MEMORY
printf("20");
    fprintf(file_malloc,"%p(1) %10llu nbt_write (line:864) write_stream.buffer\n",write_stream.buffer,(size_t)NBT_BUFFER_SIZE);         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("21");
            fprintf(file_free,"%p(3)            nbt_write (line:884) write_stream.buffer\n", write_stream.buffer);         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("22");
    fprintf(file_free,"%p(3)            nbt_write (line:964) write_stream.buffer\n", write_stream.buffer);         //DEBUG_MEM
#endif
    NBT_FREE(write_stream.buffer);
...
#ifdef DEBUG_MEMORY
printf("23");
    fprintf(file_malloc,"%p(1) %10llu tag_base (line:1039) tag\n", tag, (size_t)sizeof(nbt_tag_t));         //DEBUG_MEM
#endif
    tag->name = NULL;
    tag->name_size = 0;
    return tag;
...
#ifdef DEBUG_MEMORY
printf("24");
    fprintf(file_malloc,"%p(1) %10llu byte_ar  (line:1046) tag->tag_byte_array.value\n", tag->tag_byte_array.value, (size_t)size);         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("25");
    fprintf(file_malloc,"%p(1) %10llu string   (line:1060) tag->tag_string.value\n", tag->tag_string.value,(size_t) size +1);         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("26");
    fprintf(file_malloc,"%p(1) %10llu int_array(line:1098) tag->tag_int_array.value\n", tag->tag_int_array.value, (size_t)(size * sizeof(int32_t)));         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("27");
    fprintf(file_malloc,"%p(1) %10llu long_arra(line:1112) tag->tag_long_array.value\n", tag->tag_long_array.value, (size_t)(size * sizeof(int64_t)));         //DEBUG_MEM
#endif
...
nbt_tag_t* nbt_new_tag_float_array(float* value, size_t size)
{
    ...
    tag->type = NBT_TYPE_FLOAT_ARRAY;
    tag->tag_float_array.size = size;
    tag->tag_float_array.value = (float*)NBT_MALLOC(size * sizeof(float));
#ifdef DEBUG_MEMORY
printf("28");
    fprintf(file_malloc,"%p(1) %10llu float_arra(line:1112) tag->tag_float_array.value\n", tag->tag_float_array.value, (size_t)(size * sizeof(float)));         //DEBUG_MEM
#endif

    NBT_MEMCPY(tag->tag_float_array.value, value, size * sizeof(float));
...
nbt_tag_t* nbt_new_tag_double_array(double* value, size_t size)
{
    ...
    tag->type = NBT_TYPE_DOUBLE_ARRAY;
    tag->tag_double_array.size = size;
    tag->tag_double_array.value = (double*)NBT_MALLOC(size * sizeof(double));
#ifdef DEBUG_MEMORY
printf("29");
    fprintf(file_malloc,"%p(1) %10llu double_arra(line:1112) tag->tag_double_array.value\n", tag->tag_double_array.value, (size_t)(size * sizeof(double)));         //DEBUG_MEM
#endif

    NBT_MEMCPY(tag->tag_float_array.value, value, size * sizeof(double));
...
#ifdef DEBUG_MEMORY
printf("28");
        fprintf(file_free,"%p(3)            tag_name (line:1123) tag->name=%s\n", tag->name, tag->name);         //DEBUG_MEM
#endif
        NBT_FREE(tag->name);
    }
    tag->name_size = size;
...
#ifdef DEBUG_MEMORY
printf("29");
    fprintf(file_malloc,"%p(1) %10llu tag_name (line:1127) tag->name=%s\n", tag->name, (size_t)size + 1, tag->name);         //DEBUG_MEM
#endif
}
...
#ifdef DEBUG_MEMORY
printf("30");
    fprintf(file_realloc,"%p(2)            -> ", list->tag_list.value);         //DEBUG_MEM
#endif
    list->tag_list.value = (nbt_tag_t**)NBT_REALLOC(list->tag_list.value, (size_t)((list->tag_list.size + 1)*sizeof(nbt_tag_t*)));
    list->tag_list.value[list->tag_list.size] = value;
#ifdef DEBUG_MEMORY
printf("31");
    fprintf(file_realloc,"%p %10llu list_appe(line:1139) (size=(+1=)%llu)\n",list->tag_list.value, (size_t)((list->tag_list.size + 1)*sizeof(nbt_tag_t*)), (size_t)list->tag_list.value +1 );         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("32");
    fprintf(file_realloc,"%p(2)            -> ", compound->tag_compound.value);         //DEBUG_MEM
#endif
    compound->tag_compound.value = (nbt_tag_t**)NBT_REALLOC(compound->tag_compound.value, (size_t)((compound->tag_compound.size + 1)*sizeof(nbt_tag_t*)));
...
#ifdef DEBUG_MEMORY
printf("33");
    fprintf(file_realloc,"%p %10llu com_appen(line:1149) (size=(+1=)%llu)\n", compound->tag_compound.value, (size_t)((compound->tag_compound.size + 1)*sizeof(nbt_tag_t*)), (size_t)compound->tag_compound.size +1 );         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("34");
    fprintf(file_free,"%p(3)            nbt_free_(line:1157) tag->name=%s\n", tag->name, tag->name);         //DEBUG_MEM
#endif
    NBT_FREE(tag->name);
    switch (tag->type)
    {
...
#ifdef DEBUG_MEMORY
printf("35");
        fprintf(file_free,"%p(3)            nbt_free_(line:1157) tag->tag_byte_array.value\n", tag->tag_byte_array.value);         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("36");
        fprintf(file_free,"%p(3)            nbt_free_(line:1169) tag->tag_string.value\n", tag->tag_string.value);         //DEBUG_MEM
#endif
    ...
    case NBT_TYPE_COMPOUND:
    {
        for (size_t i = 0; i < tag->tag_compound.size; i++)
        {
            if (tag->tag_compound.value[i]!=NULL)
...
#ifdef DEBUG_MEMORY
printf("37");
        fprintf(file_free,"%p(3)            int_array(line:1193) tag->tag_int_array.value\n", tag->tag_int_array.value);         //DEBUG_MEM
#endif
...
#ifdef DEBUG_MEMORY
printf("38");
        fprintf(file_free,"%p(3)            long_arra(line:1199) tag->tag_long_array.value\n", tag->tag_long_array.value);         //DEBUG_MEM
#endif
...
printf("39");
        fprintf(file_free,"%p(3)            long_arra(line:1199) tag->tag_long_array.value\n", tag->tag_long_array.value);         //DEBUG_MEM
#endif
        NBT_FREE(tag->tag_float_array.value);
        break;
    }
    case NBT_TYPE_DOUBLE_ARRAY:
    {
#ifdef DEBUG_MEMORY
printf("40");
        fprintf(file_free,"%p(3)            double_arra(line:1199) tag->tag_double_array.value\n", tag->tag_double_array.value);         //DEBUG_MEM
#endif
        NBT_FREE(tag->tag_double_array.value);
        break;
    }
    default:
    {
...
#ifdef DEBUG_MEMORY
printf("39");
    fprintf(file_free,"%p(3)            nbt_free_(line:1219) tag\n", tag);         //DEBUG_MEM
#endif
    NBT_FREE(tag);
}
