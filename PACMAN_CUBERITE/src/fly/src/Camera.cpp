...
glm::mat4 Camera::getView()
{
    if (m_viewChanged)
    {
        m_view = glm::lookAt(
                             m_position -
                            glm::normalize(
                                glm::vec3{m_direction.x, m_direction.y, m_direction.z}) * m_distance
                            +   glm::vec3{0.f, 0.f, 1.f - m_direction.z} * (m_distance * 0.3f),  // eye

                            m_position,  // center

//                            glm::vec3{m_direction.x, m_direction.y, m_direction.z});
...
//        auto prev_dir = glm::normalize(m_direction), prev_up = glm::normalize(m_airplane.m_up);
...
//        auto cross  = glm::normalize(glm::cross(m_direction, m_airplane.m_up));
