...
sf::Image outline_image;
sf::Image outline_images[3][3];
...
bool maze_on=true;
bool lights_on=true;
bool lighten_on=true;
bool ground_on=true;
bool stone_on=true;
bool bedrock_on=true;
bool bedrock_floor_on=true;
bool white_on=true;
bool fix_on=false;
...
//                                    AY[min_y] = BlockInfo(95, 0, ret_color(255,255,255), 0 );
                ...
                printf("REGION=[%d][%d]  XX=[%3d-%3d] Y=[%3d-%3d] ZZ=[%3d-%3d] ",region_x,region_z,real_min_x,real_max_x,real_min_y,real_max_y,real_min_z,real_max_z);
...
extern std::string area;
    ...
    if (get_block) {
        sprintf(tmp, "/Saves/tovoxels/region/%s/r.%d.%d.mca", DONE, region_x, region_z);
        if ( !file_exists(tmp) ) {
            sprintf(tmp, "/Saves/tovoxels/region/r.%d.%d.mca", region_x, region_z);
        }
    }
    else
...
//        } else
        ...
        if (flushing && !(make_regions || flushing_mode)) {
            ...
            if (get_block) printf(" Region file to voxel file: %s",fname.c_str());
            else if (plotting) printf(" Plotting: %s",fname.c_str());
        }
    }
...
//    scan_image.create(512,512);
    ...
    if (mcglobal==9) {
        ...
        char fn[500];
        sprintf(fn,"%s/r.%d.%d.mp4",dir.c_str(),region_x,region_z);
        if (!file_exists(fn)) save_region_to_ffmpeg(region,region_x,region_z,fn);
    }
...
//    region_counter[region_x][region_z]++;
    ...
    if (file_exists("bedrock.on")) {
        bedrock_on=true;
        if (fff==1) printf("\n");
        fff=0;
        printf(" Bedrock=on");
    } else bedrock_on=false;

    if (file_exists("bedrock_floor.on")) {
        bedrock_floor_on=true;
        if (fff==1) printf("\n");
        fff=0;
        printf(" Bedrock floor=on");
    } else bedrock_floor_on=false;

    if (file_exists("white.on")) {
        white_on=true;
        if (fff==1) printf("\n");
        fff=0;
        printf(" White=on");
    ...
    if (file_exists("tunnels.on")) {
        tunnels_on=true;
        if (fff==1) printf("\n");
        fff=0;
        printf(" Tunnels=on");
    } else tunnels_on=false;
    ...
    if (file_exists("lighten.on")) {
        lighten_on=true;
        if (fff==1) printf("\n");
        fff=0;
        printf(" lighten=on");
    } else lighten_on=false;
...
extern bool lighten;
    /*
                                    ...
                                    else if (t==2) AY[y]=BlockInfo(2,0,0,0);
                                    else AY[y]=BlockInfo(1,0,t,0);

                                    f1=(sin(float(y+rand()%14)/27+float(xx+rand()%14)/25+float(zz+rand()%15)/49)+1.1);
                                    f2=(cos(float(y+rand()%14)/65+float(xx+rand()%14)/25+float(zz+rand()%15)/74)+1.1);
                                    f3=(sin(float(y+rand()%14)/74+float(xx+rand()%14)/64+float(zz+rand()%15)/39)+1.1);
                                    r1000x=(sin(float(xx)/445+float(zz)/199)+1.0)*100.0;
                                    r1000z=(cos(float(xx)/385+float(zz)/232)+1.0)*100.0;
                                    r1=( -0.143 * sin(f3+1.65 * (float((xx)+r1000x)/155.0 + 1.73))                     - 0.180 * sin(2.96 * (float((zz)+r1000z)/44.0+4.98))                    - 0.012 * sin(7.23 * (float((xx)+r1000x)/55.0+3.37)) + 0.088 * sin(8.07 * (float((zz)+r1000z)/39.0+4.63)) );
                                    r2=( -0.141 * sin(f1+1.75 * (float((zz)+r1000z)/123.0 + 1.63))                     - 0.160 * sin(3.96 * (float((xx)+r1000x)/48.0+5.38))                    - 0.018 * sin(6.23 * (float((zz)+r1000z)/85.0+2.17)) + 0.078 * sin(8.07 * (float((xx)+r1000x)/59.0+3.63)) );
                                    r3=( -0.123 * sin(1.35 * (float((zz)+r1000z+float(xx)+r1000x)/98.0 + 1.53))    - 0.180 * sin(2.46 * (float((xx)+r1000x)/25.0+4.28))                    - 0.017 * sin(5.23 * (float((zz)+r1000z)/65.0+3.17)) + 0.098 * sin(7.07 * (float((xx)+r1000x)/29.0+5.63)) );
                                    r4=( -0.153 * sin(1.64 * (float((xx)+r1000x)/78.0 + 1.83))                     - 0.190 * sin(2.76 * (float((zz)+r1000z-float(xx)+r1000x)/28.0+4.58))   - 0.016 * sin(4.23 * (float((xx)+r1000x)/75.0+3.47)) + 0.088 * sin(6.07 * (float((zz)+r1000z)/39.0+4.13)) );
                                    lx=int(7+7*sin(f1+f2+f3+r1*5+r2*5+r3*5+r4*5));
                    ...
                    if (!cubic && !bedrock_floor_on) {
                        if (y==height_add) {
                            if (!(rand()%200)) {
                                ...
                                if ((xx+zz)%2)
                                    AY[y] = BlockInfo(251, 0, ret_color( (128+(region_z+5)*25)%256, (128+(region_x+2)*37)%256, (128+(region_x+9)*11+(region_z+3)*31)%256),0 );
                                else
                                    AY[y] = BlockInfo(251, 0, rand_color,0 );
                            }
                            if (min_y==-1) min_y=y;
...
//                            if (y<255)  if (AX[x][z][y+1].id!=2002) AX[x][z][y+1]=BlockInfo(2003, 0, 0, 0 );
    ...
    //                                AY[y+1]=BlockInfo(9,0,8,0);
                            ...
                            if (grass_on) {
                                if (y<253) if (AY[y+1].id==0 && AY[y+2].id==0) {
    ...
    //                                }
                            ...
                            if (c==water_blue && blue_to_water_on && y<253 && !(rand()%15)) {
                                ...
                                AY[y]=BlockInfo(251,0,0,0); // make white concrete (0)
                            } else if (bedrock_on) {
                                AY[y]=BlockInfo(7,0,0,0); // make bedrock
                            } else {
...
//                        if (lighten || lighten_on) AY[y].block_light=lx;
                ...
                if (min_y<mask[x][z] || mask[x][z]==-999999) mask[x][z]=min_y;
                    ...
                    for (int z = 0; z < zl; z++) {
                        ...
                        for (int y = height_add; y < 256; y++) {
                            ...
                            if (AY[y].id!=0 && AY[y].id!=8 && AY[y].id!=46 && y>floor_y[x][z]) {
                                ...
                                float f_y=y+SEED;
                                float f_zz=zz+SEED;
                                int rr=int(2.0+2.0*sin((f_xx-f_zz+f_y)/118.77) + 8.0*(1.0+sin((f_xx+f_zz+f_y)/106.0))/2.0);
                                int b1=int( ( sin( float(f_xx)/179.83 + 0.35*sin( float(f_y)/115.83 ) ) + sin( float(f_y)/175.83 )  +  float(f_zz+ 0.27*sin( float(f_xx)/139.83 ) )/269.51 + 2.0)*300.0 + 500.0 )%300;
                                int b2=int( ( sin( float(f_xx+ 0.2*sin( float(f_zz)/245.83 ) )/199.83 ) + sin( float(f_y)/155.83 )  +  float(f_zz)/139.51 + 2.0  + 0.31*sin( float(f_y)/215.83 ))*300.0 + 500.0 )%300;
                                int b3=int( ( sin( float(f_xx+ 0.13*sin( float(f_zz)/345.83 ) )/119.83 ) + sin( float(f_y)/165.83 ) +  float(f_zz)/179.51 + 2.0  + 0.15*sin( float(f_y)/345.83 ))*300.0 + 700.0 )%350;
                                if (
                                        b3==75 || b1==140 || b2==135 ||
                                        b3==76 || b1==141 || b2==136 ||
    ...
    //                                } else if (y<floor_y[x][z]-1 - (!caves_on)*25 && y>1) {
                        ...
                        for (int z = 0; z < zl; z++) {
                            ...
                            for (int y = height_add; y < 256; y++) {
                                ...
                                if (AY[y].id!=254) {
                                    ...
                                    int r1_b,r2_b,r3_b;
                                    int r1_x_b; int r2_x_b; int r3_x_b;
                                    int r1_y_b; int r2_y_b; int r3_y_b;
                                    int r1_z_b; int r2_z_b; int r3_z_b;
                                    ...
                                    for (int n=0; n<4; n++) {
                                        ...
                                        r3_b= int(30.0 + 29.0*sin( (
                                                 0.8*( var1*f_x/1.44  + var2*f_z/2.36 )           /  (93.3 * (1.0+var1*0.4*sin((var3*f_z/1.3 + f_y/2.4 + f_x    ) / 77.0)))   +
                                                 var2*0.7*( var2*f_x/1.14  + var3*f_z/1.17 )           /  (89.4 * (1.0+var2*0.4*sin((f_z/1.7 + f_y     - var1*f_x    ) / 90.0)))   +
                                                 var3*3.10*sin( (var3*f_x/1.36 + f_z/1.55 )   /  (112.6 * (1.0+0.4*sin((var2*f_z     + f_y     + f_x/2.1) / 51.0)))   ) +
                                                 var1*4.15*sin( (f_x/1.67 + var3*f_z/1.64 )   /  (52.4 * (1.0+0.4*sin((f_z     - var1*f_y/2.7 + f_x/1.2) / 87.0)))   )
                                            )*0.8 ) );


                                        r1_b= int( 200.0*sin( (
                                                   0.7*( var2*f_z/2.23  +  f_x/2.3 )                       /   (97.3 * (1.0+var2*0.4*sin((-var3*f_z/1.2 + f_y     + f_x/1.5) / 133.0)))   +
                                                   var2*0.6*( var3*f_z/1.44  +  f_x/1.5 )                            /   (113.3 * (1.0+var3*0.4*sin((-f_z-      var2*f_y/1.6 + f_x/1.3) / 124.0)))   +
                                                   var3*1.8*sin( (var3*f_z/1.53 + f_x/1.1 )                /   (86.0 * (1.0+0.4*sin(( f_z + f_y     - var1*f_x/2.1    ) / 83.0)))   ) +
                                                   var1*2.15*sin( (f_z/1.33 + var2*f_x/1.4 )                /   (98.0 * (1.0+0.4*sin(( f_z/1.8 - var3*f_y     + f_x/1.4) / 87.0)))   )
                                            )*0.60  ) );

                                        r2_b= int( 200.0*sin( (
                                               0.7*( var3*f_x/2.02   +  f_z/2.30 )              / (126.3*(1.0+var3*0.3*sin(( f_z/1.2 + f_y/2.2 + var2*f_x      )  /117.0)))    +
                                               var3*0.6*( f_x/1.12   +  var1*f_z/1.22 )                  / (118.3*(1.0+var1*0.4*sin((-var3*f_z/1.3 - f_y     + f_x/1.7  )  /125.0)))    +
                                               var2*1.8*sin( (var2*f_x/1.0 + f_z/1.5 )          / (99.0*(1.0+0.4*sin(( f_z/1.32 - f_y     + var3*f_x      )  /85.0)))    ) +
                                               var1*2.15*sin( (f_x/1.5 + var3*f_z/1.7 )          / (83.0*(1.0+0.4*sin(( var1*f_z/1.5 + f_y/2.1 - f_x      )  /83.0)))    )
                                            )*0.60 ) );

                                        if (n==0)       {r3_x=r3+height_add+1;      r1_x=r1+255; r2_x=r2+255;   r3_x_b=r3_b+height_add+1;      r1_x_b=r1_b+255; r2_x_b=r2_b+255;  }
                                        else if (n==1)  {r3_y=r3+height_add+1;      r1_y=r1+255; r2_y=r2+255;   r3_y_b=r3_b+height_add+1;      r1_y_b=r1_b+255; r2_y_b=r2_b+255;  }
                                        else if (n==2)  {r3_z=r3+height_add+1;      r1_z=r1+255; r2_z=r2+255;   r3_z_b=r3_b+height_add+1;      r1_z_b=r1_b+255; r2_z_b=r2_b+255;  }
                                        else            {r3=  r3+height_add+1;      r1=r1+255;   r2=r2+255;     r3_b=  r3_b+height_add+1;      r1_b=r1_b+255;   r2_b=r2_b+255;    }
                                    ...
                                    if (!hit) {
                                        if ( !((r3_b>y && r3_x_b>y && r3_y_b>y && r3_z_b>y) || (r3_b<y && r3_x_b<y && r3_y_b<y && r3_z_b<y)) ) {
                                            if (
                                                    ((r1_b>x && r1_x_b>x && r1_y_b>x && r1_z_b>x) || (r1_b<x && r1_x_b<x && r1_y_b<x && r1_z_b<x))
                                                 &&
                                                    ((r2_b>z && r2_x_b>z && r2_y_b>z && r2_z_b>z) || (r2_b<z && r2_x_b<z && r2_y_b<z && r2_z_b<z))
                                                ) {
                                                hit=false;
                                            } else hit=true;
                                        }
        ...
        //                                                    if ( (xxx==0 && zzz==0 && abs(yyy)==2) || (xxx==0 && abs(zzz)==2 && yyy==0) || (abs(xxx)==2 && zzz==0 && yyy==0) || ( abs(xxx)+abs(yyy)+abs(zzz) <= 1  ) ) {
                                                                *hoppa=BlockInfo(46,0,0,0);
                                                                ...
                                                                if (dd2<3.5) {
                                                                    if (!(rand()%2000)) *hoppa=BlockInfo(89, 0, 0, 0);
...
//                   float r4 = -0.153 * sin(1.64 * (ran7*float(xx)/24.0 + 1.83)) - 0.194 * sin(2.76 * (ran7*float(zz-xx)/58.0+4.58)) - 0.016 * sin(4.23 * (float(xx)/25.0+3.17)) + 0.088 * sin(6.07 * (float(zz)/39.0+4.63));
                   ...
                   float r2=( -0.141 * sin(1.75 * (float((zz)+r1000z)/123.0 + 1.63))                     - 0.160 * sin(3.96 * (float((xx)+r1000x)/48.0+5.38))                    - 0.018 * sin(6.23 * (float((zz)+r1000z)/85.0+2.17)) + 0.078 * sin(8.07 * (float((xx)+r1000x)/59.0+3.63)) );
                   float r3=( -0.123 * sin(1.35 * (float((zz)+r1000z+float(xx)+r1000x)/98.0 + 1.53))    - 0.180 * sin(2.46 * (float((xx)+r1000x)/25.0+4.28))                    - 0.017 * sin(5.23 * (float((zz)+r1000z)/65.0+3.17)) + 0.098 * sin(7.07 * (float((xx)+r1000x)/29.0+5.63)) );
                   float r4=( -0.153 * sin(1.64 * (float((xx)+r1000x)/78.0 + 1.83))                     - 0.190 * sin(2.76 * (float((zz)+r1000z-float(xx)+r1000x)/28.0+4.58))   - 0.016 * sin(4.23 * (float((xx)+r1000x)/75.0+3.47)) + 0.088 * sin(6.07 * (float((zz)+r1000z)/39.0+4.13)) );
                   ...
                   r3+=f3*0.3*( -0.123 * sin(f7*1.55 * (float(zz)/30.0 + 1.53)) - f4*f1*0.180 * sin(2.46 * (float(xx)/25.0+f8*f4*4.28)) - 0.017 * sin(5.23 * (float(zz)/35.0+3.17)) + 0.098 * sin(f5*7.37 * (float(xx)/29.0+5.63)));
                   r4+=f4*0.3*( -0.153 * sin(f8*1.60 * (float(xx)/24.0 + 1.83)) - f1*f2*0.192 * sin(2.76 * (float(zz)/28.0+f6*f1*4.58)) - 0.016 * sin(4.23 * (float(xx)/25.0+3.17)) + 0.083 * sin(f6*6.07 * (float(zz)/47.0+4.53)));

                    float r7 = ((r1 + r2 + r3 + r4)+0.55)*50.0;
...
//                                    if ( (x+a<512 && floor_y[x+a][z]!=-999999 && (id==1 || id==2 || id==3 || id==14 || id==89))
//                                        || (x+a==512 && floor_y[511][z]==999999)) {
                                    if ( (x+a<512 && floor_y[x+a][z]!=-999999 && (id==1 || id==2 || id==3 || id==14 || id==89))
                                        || (x+a==512 && floor_y[511][z]==-999999)) {
                                        a--;
                                        while (a>0) {
...
//                                                    floor_y[x+a][z]=y;
                                                ...
                                                if (y+region_floor*256>floor_y[x+a][z]) scan_image.setPixel(x+a,z,sf::Color(255,255,0,255));
                                                else {
                            ...
                            if (x<511) {
                                ...
                                if ((id==1 || id==2 || id==3 || id==14 || id==89) ) {
                                    ...
                                    if ( (x+a>=0 && floor_y[x+a][z]!=-999999 && (id==1 || id==2 || id==3 || id==14 || id==89))
                                        || (x+a<0 && floor_y[0][z]==-999999)) {
                                        a++;
                                        while (a<0) {
                                            if (mask[x+a][z]) {
                                                AX[x+a][z][y] = BlockInfo(1, 0, 0, 0 );
//                                                floor_y[x+a][z]=y;
                            ...
                            if (z>0) {
                                ...
                                if ((id==1 || id==2 || id==3 || id==14 || id==89) ) {
                                    ...
                                    if ( (z+a<512 && floor_y[x][z+a]!=-999999 && (id==1 || id==2 || id==3 || id==14 || id==89))
                                        || (z+a==512 && floor_y[x][511]==-999999)) {
                                        a--;
                                        while (a>0) {
                                            if (mask[x][z+a]) {
...
//                                                floor_y[x][z+a]=y;
                                                cured=false; added++; added_loop++; added_y++;
                                                if (y+region_floor*256>floor_y[x][z+a]) scan_image.setPixel(x,z+a,sf::Color(255,255,0,255));
                                                else {
                            ...
                            if (z<511) {
                                ...
                                if ((id==1 || id==2 || id==3 || id==14 || id==89) ) {
                                    ...
                                    if ( (z+a>=0 && floor_y[x][z+a]!=-999999 && (id==1 || id==2 || id==3 || id==14 || id==89))
                                        || (z+a<0 && floor_y[x][0]==-999999)) {
                                        a++;
                                        while (a<0) {
                                            if (mask[x][z+a]) {
                                                AX[x][z+a][y] = BlockInfo(1, 0, 0, 0 );
//                                                floor_y[x][z+a]=y;
                            ...
                            if (num>0) {
                                ...
                                sf::Color mix;
                                mix=sf::Color(255,255,0,255);
                                if (y+region_floor*256>floor_y[x][z]) scan_image.setPixel(x,z,mix);
                                else {
...
//    int round_height2=0;
    if (mcglobal5==0) {
        ...
        read_one_plot_WINMAIN(region_x*512-5, region_z*512-5, &image_plot);
        round_height=0;
    } else if (mcglobal5==10) {
        local_mcglobal2=local_mcglobal2-3;
        local_mcglobal3=local_mcglobal3-19;
...
//moved
/*
    if (mcglobal5==0 || mcglobal5==1 || mcglobal5==3 || mcglobal5==5 || mcglobal5==15 || mcglobal5==10) {
...
*/
    if (outline) {
        for (int x = 0; x < 5; x++) {
            for (int z = 0; z < 5; z++) {
                if (outline_images[0][0].getPixel(x+512-5,z+512-5).r!=0)    set_it(x,z,image_plot_b,image_plot_c);
                if (outline_images[0][2].getPixel(x,z+512-5).r!=0)          set_it(x+512+5,z,image_plot_b,image_plot_c);
                if (outline_images[2][0].getPixel(x+512-5,z).r!=0)          set_it(x,z+512+5,image_plot_b,image_plot_c);
                if (outline_images[2][2].getPixel(x,z).r!=0)                set_it(x+512+5,z+512+5,image_plot_b,image_plot_c);
            }
        }
        for (int x = 0; x < 512; x++) {
            for (int z = 0; z < 5; z++) {
                if (outline_images[0][1].getPixel(x,z+512-5).r!=0) set_it(x+5,z,image_plot_b,image_plot_c);
                if (outline_images[2][1].getPixel(x,z).r!=0) set_it(x+5,z+512+5,image_plot_b,image_plot_c);
            }
        }
        for (int x = 0; x < 5; x++) {
            for (int z = 0; z < 512; z++) {
                if (outline_images[1][0].getPixel(x+512-5,z).r!=0) set_it(x,z+5,image_plot_b,image_plot_c);
                if (outline_images[1][2].getPixel(x,z).r!=0) set_it(x+512+5,z+5,image_plot_b,image_plot_c);
            }
        }
        ...
        for (int x = 0; x < xl; x++) {
            toggle2();
            for (int z = 0; z < zl; z++) {
                if (outline_images[1][1].getPixel(x,z).r!=0) {
                    ...
                    if (col.r==0) {
                        for (int xxx=-1; xxx<=1; xxx++) {
                            for (int zzz=-1; zzz<=1; zzz++) {
                                if (xxx!=0 || zzz!=0) {
                                    if (x+xxx>=0 && x+xxx<512+10 && z+zzz>=0 && z+zzz<512+10) {
//                                        if (!(image_plot2.getPixel(x+xxx*2,z+zzz*2).r==0)) {
...
//new2
        for (int x = 0; x < 512+10; x++) {
            int xx=x+region_x*512;
            for (int z = 0; z < 512+10; z++) {
                int zz=z+region_z*512;
                if ( ((((int)xx+1)%3)==0 && (((int)zz+1)%3)==0)) {
                    bool hit=false;
                    for (int xxx=-1; xxx<=1; xxx++) {
                        for (int zzz=-1; zzz<=1; zzz++) {
                            if (x+xxx>=0 && x+xxx<512+10 && z+zzz>=0 && z+zzz<512+10) {
                                if (image_plot_b.getPixel(x+xxx,z+zzz).r!=0) {
                                    hit=true;
                                    break;
                                }
                            }
                    ...
                    if (hit) {
                        for (int xxx=-1; xxx<=1; xxx++) {
                            for (int zzz=-1; zzz<=1; zzz++) {
                                if (x+xxx>=0 && x+xxx<512+10 && z+zzz>=0 && z+zzz<512+10) {
                                    image_plot_b.setPixel(x+xxx,z+zzz,sf::Color(255,255,255,255));
                                }
                            }
                        }
                    }
            ...
            for (int z = 0; z < 512+10; z++) {
                ...
                if ( ((((int)xx+1)%3)==0 && (((int)zz+1)%3)==0)) {
                    ...
                    for (int xxx=-1; xxx<=1; xxx++) {
                        for (int zzz=-1; zzz<=1; zzz++) {
                            if (x+xxx>=0 && x+xxx<512+10 && z+zzz>=0 && z+zzz<512+10) {
                                if (image_plot_c.getPixel(x+xxx,z+zzz).r==0) {
                                    hit=true;
                                    break;
                                }
                            }
                    ...
                    if (hit) {
                        for (int xxx=-1; xxx<=1; xxx++) {
                            for (int zzz=-1; zzz<=1; zzz++) {
                                if (x+xxx>=0 && x+xxx<512+10 && z+zzz>=0 && z+zzz<512+10) {
                                    image_plot_c.setPixel(x+xxx,z+zzz,sf::Color(0,0,0,0));
                                }
                            }
                        }
                    }
        ...
        for (int x = 0; x < 512+10; x++) {
            for (int z = 0; z < 512+10; z++) {
                if ( (mcglobal5==0) && (sqrt((256-x)*(256-x)+(256-z)*(256-z))<30) ) {
                    image_plot.setPixel(x,z,sf::Color(255,255,255,255));
                    image_plot3.setPixel(x,z,sf::Color(255,255,255,255));
                } else if ( (mcglobal5!=0 && mcglobal5!=2 && mcglobal5!=10) || ((sqrt((256+5-x)*(256+5-x)+(256+5-z)*(256+5-z))<179) ) ) {
                    if (image_plot_b.getPixel(x,z).r==0) {
                        image_plot.setPixel(x,z,sf::Color(0,0,0,0));
                        image_plot3.setPixel(x,z,sf::Color(0,0,0,0));
                    }
                ...
                if (col.r==0) {
                    for (int xxx=-1; xxx<=1; xxx++) {
                        for (int zzz=-1; zzz<=1; zzz++) {
                            if (xxx!=0 || zzz!=0) {
                                if (x+xxx>=0 && x+xxx<512+10 && z+zzz>=0 && z+zzz<512+10) {
//                                    if (!(image_plot2.getPixel(x+2*xxx,z+2*zzz).r)==0) {
                                        image_plot3.setPixel(x+xxx,z+zzz, col);
//                                    }
//                                    image_plot.setPixel(x+xxx,z+zzz, col);
                                }
    ...
    for (int x = 1; x < 511+10; x++) {
        for (int z = 1; z < 511+10; z++) {
            if (image_plot2.getPixel(x,z).r==0) {
                if (
                        (
                        image_plot2.getPixel(x-1,z+1).r==0 &&
                        image_plot2.getPixel(x-1,z  ).r==0 &&
                        image_plot2.getPixel(x-1,z-1).r==0 &&
                        image_plot2.getPixel(x  ,z-1).r==0 &&
                        image_plot2.getPixel(x+1,z-1).r==0 &&
                        image_plot2.getPixel(x+1,z  ).r!=0 &&
                        image_plot2.getPixel(x+1,z+1).r!=0 &&
                        image_plot2.getPixel(x  ,z+1).r!=0
        ...
        for (int z = 1; z < 511+10; z++) {
            if (image_plot2.getPixel(x,z).r!=0) {
                if ((image_plot2.getPixel(x-1,z).r==0) &&  (image_plot2.getPixel(x+1,z).r==0)  ) {
                        image_plot.setPixel(x,z,sf::Color(0,0,0,0));
//                        image_plot.setPixel(x-1,z,sf::Color(255,255,255,255));
//                        image_plot.setPixel(x+1,z,sf::Color(255,255,255,255));
                }
                if ((image_plot2.getPixel(x,z-1).r==0 ) && (image_plot2.getPixel(x,z+1).r==0)  ) {
                        image_plot.setPixel(x,z,sf::Color(0,0,0,0));
//                        image_plot.setPixel(x,z-1,sf::Color(255,255,255,255));
//                        image_plot.setPixel(x,z+1,sf::Color(255,255,255,255));
                }
            }
        }
    ...
    for (int x = 1; x < 511+10; x++) {
        for (int z = 1; z < 511+10; z++) {
            if (image_plot2.getPixel(x,z).r!=0) {
                if ((image_plot2.getPixel(x-1,z).r==0) &&  (image_plot2.getPixel(x+1,z).r==0)  ) {
                        image_plot3.setPixel(x,z,sf::Color(0,0,0,0));
//                        image_plot3.setPixel(x-1,z,sf::Color(255,255,255,255));
//                        image_plot3.setPixel(x+1,z,sf::Color(255,255,255,255));
                }
                if ((image_plot2.getPixel(x,z-1).r==0 ) && (image_plot2.getPixel(x,z+1).r==0)  ) {
                        image_plot3.setPixel(x,z,sf::Color(0,0,0,0));
//                        image_plot3.setPixel(x,z-1,sf::Color(255,255,255,255));
//                        image_plot3.setPixel(x,z+1,sf::Color(255,255,255,255));
                }
            }
        }
...
void get_surroundings(MCRegion& region_cur, int region_x, int region_z, std::string fname, MCRegion region_surroundings[3][3] ) {
    if (fname.find_last_of(".") != std::string::npos) fname=fname.substr(0,fname.find_last_of("."))+".mp4";
    outline_images[1][1].create(512,512);
    MCRegion region(0, 0, 0, 512, 512, 256);
    if (!file_exists(fname.c_str())) {
        save_region_to_ffmpeg(region_cur,region_x,region_z,fname);
        outline_images[1][1].copy(outline_image,0,0);
    } else {
    ...
    for (int r_x=region_x-1; r_x<=region_x+1; r_x++) {
        for (int r_z=region_z-1; r_z<=region_z+1; r_z++) {
            if (!(r_z==region_z && r_x==region_x)) {
                ...
                if (ret==0) {
                    outline_images[z_pos+1][x_pos+1].create(512,512);
                    outline_images[z_pos+1][x_pos+1].copy(outline_image,0,0);
                    BlockInfo*** AX=region.A;

                    if (z_pos==-1) {
                        if (x_pos==-1)      copy_region(AX, 512-5, 512-5, 0,   5,   5, 256, what_AX, 0, 0, 0);
                        else if (x_pos==0)  copy_region(AX,     0, 512-5, 0, 512,   5, 256, what_AX, 0, 0, 0);
                        else                copy_region(AX,     0, 512-5, 0,   5,   5, 256, what_AX, 0, 0, 0);
                    } else if (z_pos==0) {
                        if (x_pos==-1)      copy_region(AX, 512-5,     0, 0,   5, 512, 256, what_AX, 0, 0, 0);
                        else if (x_pos==1)  copy_region(AX,     0,     0, 0,   5, 512, 256, what_AX, 0, 0, 0);
                    } else {
                        if (x_pos==-1)      copy_region(AX, 512-5,     0, 0,   5,   5, 256, what_AX, 0, 0, 0);
                        else if (x_pos==0)  copy_region(AX,     0,     0, 0, 512,   5, 256, what_AX, 0, 0, 0);
                        else                copy_region(AX,     0,     0, 0,   5,   5, 256, what_AX, 0, 0, 0);
                    }
                } else {
                    outline_images[z_pos+1][x_pos+1].create(512,512,sf::Color(255,255,255,255));
                    if (z_pos==-1) {
                        if (x_pos==-1)      fill_region(  5,   5, 256, what_AX, 0, 0, 0);
                        else if (x_pos==0)  fill_region(512,   5, 256, what_AX, 0, 0, 0);
                        else                fill_region(  5,   5, 256, what_AX, 0, 0, 0);
                    } else if (z_pos==0) {
                        if (x_pos==-1)      fill_region(  5, 512, 256, what_AX, 0, 0, 0);
                        else if (x_pos==1)  fill_region(  5, 512, 256, what_AX, 0, 0, 0);
                    } else {
                        if (x_pos==-1)      fill_region(  5,   5, 256, what_AX, 0, 0, 0);
                        else if (x_pos==0)  fill_region(512,   5, 256, what_AX, 0, 0, 0);
                        else                fill_region(  5,   5, 256, what_AX, 0, 0, 0);
...
//    std::string FFMPEG="ffmpeg.exe -y -loglevel error -stats -hide_banner -f rawvideo -vcodec rawvideo -pix_fmt rgba -s %dx%d -r 30 -i - -an -c:v png %s";
    ...
    outline_image.create(512,512,sf::Color(0,0,0,0));
    unsigned char* frame_out=(unsigned char*) one_image.getPixelsPtr();
    unsigned char* frame_outline=(unsigned char*) outline_image.getPixelsPtr();
    sf::Color one_color;
    BlockInfo* A;
    int show=10;
    for (int y = 0; y < 256; y++) {
...
//                }
                if (y>90) {
                    if (A->id!=0) {
                        outline_image.setPixel(x,z,sf::Color(255,255,255,255));
                    }
                }
...
*/
    ...
    fwrite(frame_outline, 1, (512)*(512)*4, pipeout);
...
//        if (qcount_i != 0) printf("different y=%d!!!\n",y);
    ...
    num_bytes=fread(frame_outline, 1, (512)*(512)*4, pipein);
//        one_image.loadFromMemory(frame_in,512*512*4);
