    ...
    for (size_t m = 0; m < materials.size(); m++) {
      ...
      if (mp->diffuse_texname.length() > 0) {
        ...
        if (strstr(texture_filename.c_str(), ".png") != NULL) {
            ...
            image = stbi_changed_load(texture_filename.c_str(), &w, &h, &comp, stbi_changed_default);
        }



//          unsigned char* image =
//              stbi_changed_load(texture_filename.c_str(), &w, &h, &comp, stbi_changed_default);
          if (!image) {
...
//                        glTexSubImage2D(GL_TEXTURE_2D, 1, 0, 0, std::max(1, (w / 2)), std::max(1, (h / 2)), GL_RGB, GL_UNSIGNED_BYTE, image);
//                        glTexSubImage2D(GL_TEXTURE_2D, 2, 0, 0, std::max(1, (w / 4)), std::max(1, (h / 4)), GL_RGB, GL_UNSIGNED_BYTE, image);
//                        glTexSubImage2D(GL_TEXTURE_2D, 3, 0, 0, std::max(1, (w / 8)), std::max(1, (h / 8)), GL_RGB, GL_UNSIGNED_BYTE, image);
...
//                        glTexSubImage2D(GL_TEXTURE_2D, 1, 0, 0,  std::max(1, (w / 2)), std::max(1, (h / 2)), GL_RGBA, GL_UNSIGNED_BYTE, image);
//                        glTexSubImage2D(GL_TEXTURE_2D, 2, 0, 0,  std::max(1, (w / 4)), std::max(1, (h / 4)), GL_RGBA, GL_UNSIGNED_BYTE, image);
//                        glTexSubImage2D(GL_TEXTURE_2D, 3, 0, 0,  std::max(1, (w / 8)), std::max(1, (h / 8)), GL_RGBA, GL_UNSIGNED_BYTE, image);
...
//        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
        ...
        if (smooth)
          ...
          if (!png) stbi_changed_image_free(image);
...
//color out
        o.numTriangles = buffer.size() / (3 + 3 + 2) /
...
//            window.create(sf::VideoMode(1920,1080), window_title, sf::Style::Fullscreen, contextSettings);
    ...
    */
    ...
    //        glFrustum(-ratio, ratio, -1.f, 1.f, 1.f, 500.f);
            ...
            while ( (loaded == 1 ? printf("Koekoek\n") : 0) || (window.isOpen() && exit_thread==0))
            {
                if (round<0) loaded=0;
                else round--;
