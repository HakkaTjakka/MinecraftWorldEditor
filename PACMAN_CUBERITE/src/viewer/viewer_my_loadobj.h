...
extern void init_collect_object();
extern void collect_object();
...
// not anymore, lat lon values per octant for large scale conversion....
            if (area=="Utrecht" || area=="Holland") { // for rotation RECALC_BMIN_BMAX2(buffers, bmin, bmax, lat, lon); when converting to .nbt instead of lat_global and lon_global for all octants
...
//        char line[100];
        ...
        if (!NBT_LOADED) {
//tuuttuut
//            int n_v=(int)(attrib.vertices.size()) / 3;
//            for (size_t v = 0; v < n_v; v++) {
//                attrib.vertices[3 * v + 0]-=14336;
//                attrib.vertices[3 * v + 1]-=0;
//                attrib.vertices[3 * v + 2]-=9216;
//            }
...
*/
                        ...
                        cc[0][k] = attrib.colors[3 * f0 + k];
                        cc[1][k] = attrib.colors[3 * f1 + k];
                        cc[2][k] = attrib.colors[3 * f2 + k];
...
//                    float n[3][3];
                    ...
                    for (int k = 0; k < 3; k++)
                    {
                        ...
                        buffer.push_back(cc[0][0]);  buffer.push_back(cc[0][1]);  buffer.push_back(cc[0][2]);
    //color out
...
//
//                        } else {
                            c[3] = {n[k][0] * normal_factor + diffuse[0] * diffuse_factor,
...
*/
                            if ((play_num==1 && voxels_total.size()>0) || voxels_total.size() > 100000000 || voxels.size() > 100000000) {
...
//    write_nbt_file(nbt_filename.c_str(), tag_object, NBT_WRITE_FLAG_USE_ZLIB);
...
//bottom right:X=3119492.6521769455 , Z=-5360902.305020416
    ...
    } else if ((area=="Utrecht" || area=="Holland") && mirror!=0) {
        vertical =bmax_total[0]- bmin_total[0];
...
//                bmin_total[0]=-80.0;
//                bmax_total[0]=60.0;
            } else if (area=="Utrecht" || area=="Holland") {
...
//        bmax_octant[0]=bmax_total[0];
    ...
    if (area=="Utrecht" || area=="Holland") block_scale_x=blocks_x/(bmax_o[1]-bmin_o[1]);
    ...
    if (area=="Utrecht" || area=="Holland") block_scale_z=blocks_z/(bmax_o[2]-bmin_o[2]);
    ...
    if (area=="Utrecht" || area=="Holland") block_scale_avg=1.0;
...
//        pixels_per_block_glob=0.5*pixels_per_block;
...
extern std::vector<Voxel> one_object_voxels;
//jojo
    printf("\rObject Voxels: %9d  Hits: %8d  ", one_object_voxels.size(), check_tot_hits);
//    printf("\rVoxels: %9d  Hits: %8d  ", voxels.size(), check_tot_hits);
    printf("Buffers: %3d  Materials: %3d  Total faces: %8d Collecting: ",buffers.size(),materials.size(),tot_faces);
    collect_object();
    printf("\nVoxels Total:  %9d\n", voxels.size());
...
//                            printf("DIR: /saves/test/region/%s/%s\n",de_ROOT->d_name,de->d_name);
                            ...
                            if ((strstr(de->d_name, ".mca")) != NULL) {
                                ...
                                char fullname[1000];
                                sprintf(fullname,"%s/%s",floor_dir,de->d_name);
                                struct stat stat_buffer;
                                int exist = stat(fullname,&stat_buffer);
                                if (exist!=0) {
                                    printf("ERR: stat(\"%s\",&stat_buffer)\n",fullname);
                                    one_cubic_region.st_mtime=0;
                                }
                                else one_cubic_region.st_mtime=stat_buffer.st_mtime;
    ...
    */
                if (cubic_regions_mapped.size()>0) {
                    ...
                    int prev_z=-99999999;
                    it = cubic_regions_mapped.begin();
                    bool only_new_mca_files=false;
                    if (file_exists("plot_tiles_only_new_mca_files.on")) only_new_mca_files=true;
                    while(it != cubic_regions_mapped.end()) {
                        int x=it->second.x;
                        int z=it->second.z;
...
//                        if (!flushing && (mirror==3 || does_exist==false) ) {
                        ...
                        time_t st_mtime=0;
                        struct stat stat_buffer;
                        int exist = stat(picture_file,&stat_buffer);
                        if (exist!=0) printf("\nERR: stat(\"%s\",&stat_buffer)\n",picture_file);
                        else st_mtime=stat_buffer.st_mtime;
                        char tmp[1000];
                        sprintf(tmp, "/Saves/Test/region/done%d/r.%d.%d.mca", region_floor, x, z);
                        bool rescan=false;
                        if (it->second.st_mtime > stat_buffer.st_mtime) {
                            printf("Date (%s) file %s newer than\n",
                                   std::ctime(&it->second.st_mtime), tmp);
                            printf("Date (%s) file %s, rescanning\n",
                                   std::ctime(&stat_buffer.st_mtime), picture_file);
                            rescan=true;
                        }
...
//leuk            Voxel last=voxels[5];
            ...
            for (size_t n=0; n<=vend; n++) {
                ...
                if (last.x==now.x && last.y==now.y && last.z==now.z) {
                    ...
                    if (now.face!=0) {
//jojo
                        if (total_pos.face<0 && now.face>0) total_pos.face=now.face;
                        else if ( (total_pos.face>=0 && now.face>0) || (total_pos.face<=0 && now.face<0)  ){
                    ...
                    if (n==vend) {
                        ...
                        if (total_pos.l!=0) total_pos.face=0;
                        if (total_pos.face>=0) voxels_temp.push_back(total_pos);
                    }
                } else {
                    total_pos.status=0; //new
                    if (total_pos.l!=0) total_pos.face=0;
        ...
        if (make_schematic) {
            char fname[200]; sprintf (fname,"../cut/%s.vox",fn.c_str());
...
//jojo
                    ...
                    if (n==vend) {
                        if (total_pos.l!=0) total_pos.face=0;
                        if (total_pos.face>=0) voxels_total.push_back(total_pos);
                    }
                } else {
                    if (total_pos.l || total_pos.face>0) voxels_total.push_back(total_pos);
                    last=now;
                    total_pos=last;
                    if (n==vend) {
                        if (total_pos.l!=0) total_pos.face=0;
...
//                    prev_z_mod=(int)(((LONG64)prev.z+100000*512)%512);
                    ...
                    if (prev.l!=0)
                        ...
                        scan_image.setPixel(prev_y_mod , prev_z_mod,sf::Color(128,128,255,255));
...
//            printf("\nVOXELS_TOTAL: debug: XX=[%3d-%3d] Y=[%4d-%4d] ZZ=[%3d-%3d]\n",debug_min_y,debug_max_y,debug_min_x,debug_max_x,debug_min_z,debug_max_z);
            ...
            for (size_t n=0; n < vector_hit_regions.size(); n++) {
                ...
                if (hit_one->index6>0) {
                    ...
                    printf("XX=[%3d-%3d] Y=[%4d-%4d] ZZ=[%3d-%3d]  ",
...
//                                        prev_z_mod=(int)(((LONG64)u.z+100000*512)%512);
                                        ...
                                        if (u.l>0 || u.face!=0) {
                                            ...
                                            if (u.l) {
                                                ...
                                                region_block[off_x+3]= 128;
                                            } else region_block[off_x+3]= (unsigned char)( 128 + u.face );
                                        } else {
                                            printf("ERROR: u.l=0\n");
                                        }
                                    } else {
...
//                                        if (u.x>=0 && u.x<230 && u.l>0) {
                                            ...
                                            if (u.l) {
                                                ...
                                                if (u.face>0) region_block[off_x+3]=128+1;
                                                else if (u.face<0) region_block[off_x+3]=128-1;
                                            }
//                                            region_block[off_x+3]=u.l;
                                        } else if (u.l>0 || u.face!=0) {
//koekkoek
//                                            u.x=u.x-10;
                                            if (u.x<1) {
...
//                                                region_block[off_x+3]=u.l;
                                                ...
                                                TOPPED_MAX++;
                                                float togo=u.x-245.0;
                                                float new_real_y=245.0+togo/(1.0+(togo/10.0));
                                                if (new_real_y>=256.0) new_real_y=255.0;
...
void region_voxel_files_to_region_files(bool display_only) {
    ...
    while ((de = readdir(dr)) != NULL) {
        if ((strstr(de->d_name, ".vox")) != NULL) {
            ...
            if (num==2) {
                if (make_object)
                    printf("Converting %s to Wavefront .obj/.mtl/textures file(s)\n",de->d_name);
                else
                ...
                sprintf(picture_file,"%s",name.c_str());
//                continue;
            }
            plot_only=0;
            area=picture_file;
...
//            bool ret=true;
//            if (!make_object) {
//                make_region_from_voxel(x,z);
//            } else ret=load_voxels();
...
void one_region_voxel_files_to_region_files(bool display_only, char* voxel_filename) {
    ...
    printf("LOADING .VOX FILES ../cut/%s\n", voxel_filename);
    ...
    if ((strstr(voxel_filename, ".vox")) != NULL) {
        ...
        if (num==2) {
            if (make_object) {
                printf("Converting ../cut/%s voxel region file to ../objects/r.%d.%d.obj wavefront\n",voxel_filename,x,z);
            } else {
