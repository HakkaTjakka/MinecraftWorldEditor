...
extern "C" Allocator* xallocator_get_allocator(size_t size)
{
	...
	if (blockSize > 256 && blockSize <= 396)
		...
		blockSize = 768;
	else if (blockSize > 768 && blockSize <= 1024)
		blockSize = 1024;
	else
...
size_t xalloc_total_memory=0;
int xalloc_stats_quiet=0;
...
extern "C" void xalloc_stats()
{
	...
	for (INT i=0; i<MAX_ALLOCATORS; i++)
	{
        ...
        if (!xalloc_stats_quiet) printf("                                                                                                                                                  \r");
        if (!xalloc_stats_quiet) printf("Name: %10s  ",     _allocators[i]->GetName());
        if (!xalloc_stats_quiet) printf("Block Size: %13s    ",        pretty_number(_allocators[i]->GetBlockSize() ) );
        if (!xalloc_stats_quiet) printf("Block Count: %13s    ",       pretty_number(_allocators[i]->GetBlockCount() ) ) ;
        if (!xalloc_stats_quiet) printf("Blocks In Use: %13s    ",     pretty_number(_allocators[i]->GetBlocksInUse() ) );
        if (!xalloc_stats_quiet) printf("Total=%14s\n",                pretty_number(_allocators[i]->GetBlockSize()*_allocators[i]->GetBlocksInUse() ) ) ;
        total+=_allocators[i]->GetBlockSize()*_allocators[i]->GetBlocksInUse();
	}
    xalloc_total_memory=total;
    if (!xalloc_stats_quiet) printf("Total=%15s\n",pretty_number(total) );
...
extern "C" void xalloc_stats_to_file(char* name)
{
	...
	for (INT i=0; i<MAX_ALLOCATORS; i++)
	{
        ...
        fprintf(file,"Name: %10s  ",                _allocators[i]->GetName());
        fprintf(file,"Block Size: %13s    ",        pretty_number(_allocators[i]->GetBlockSize() ) );
        fprintf(file,"Block Count: %13s    ",       pretty_number(_allocators[i]->GetBlockCount() ) ) ;
        fprintf(file,"Blocks In Use: %13s    ",     pretty_number(_allocators[i]->GetBlocksInUse() ) );
        fprintf(file,"Total=%14s\n",                pretty_number(_allocators[i]->GetBlockSize()*_allocators[i]->GetBlocksInUse() ) ) ;
        total+=_allocators[i]->GetBlockSize()*_allocators[i]->GetBlocksInUse();
	}
    printf("Total=%15s\n",pretty_number(total) );
...
size_t memory_last_total=0;
size_t memory_total=0;
long long signed int memory_difference=0;
extern size_t get_mem();

extern "C" void xalloc_get_tot_mem()
{
#ifdef XALLOC
	lock_get();
	memory_last_total=memory_total;
	memory_total=0;
	for (INT i=0; i<MAX_ALLOCATORS; i++) {
		if (_allocators[i] != 0)
            memory_total+=_allocators[i]->GetBlockSize()*_allocators[i]->GetBlocksInUse();
	}
	lock_release();
	memory_difference=memory_total-memory_last_total;
#else
//    printf("nono!\n");
	memory_last_total=memory_total;
    memory_total=get_mem();
	memory_difference=memory_total-memory_last_total;
