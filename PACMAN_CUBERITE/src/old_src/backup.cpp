...
//        printf("Here 3\n");
        if (size==1) {
            total_size[(long long) userdata]=NBT_BUFFER_SIZE;
            intern[(long long) userdata]=NULL;                              /* in case not freed, pointer to previous address invallid, maybe in use.....so, new memory allocated, old not changed... */
            intern[(long long) userdata] = (uint8_t*)NBT_MALLOC(total_size[(long long) userdata]);
            current_position[(long long) userdata]=0;
//            printf("Here 4\n");
            return 0;
        } else {
            intern[(long long) userdata] = (uint8_t*)NBT_REALLOC(intern[(long long) userdata], current_position[(long long) userdata]+1);  /* adjust to real size... */
//            printf("Here 5\n");
            return(current_position[(long long) userdata]+1);
        }
    }
//    printf("Here 6\n");
    if (size==0) return 0;
//    printf("Here 7\n");
    ...
    if (current_position[(long long) userdata]+size>=total_size[(long long) userdata]) {
//        printf("Here 8\n");
        total_size[(long long) userdata]*=2;
        intern[(long long) userdata] = (uint8_t*)NBT_REALLOC(intern[(long long) userdata], total_size[(long long) userdata]);
//        printf("Here 9\n");
    }
//    printf("Here 10\n");
    NBT_MEMCPY(intern[(long long) userdata]+current_position[(long long) userdata],data,size);
//    printf("Here 11\n");
    current_position[(long long) userdata]+=size;
//    printf("Here 12\n");
    return size;
}
...
//    printf("Called writer_write_to_memory, getting size=%lld\n",size);
    ...
    depth--;
    return (intern[(long long)depth+1]); /* NBT_FREE returned pointer after use.... */
}
...
//    printf("Writing r.0.0/Original 0.nbt\n");
    write_nbt_file("r.0.0/Original 0.nbt", tag_in, NBT_WRITE_FLAG_USE_RAW);
...
//                                                                                                                      location    location2   tag_in  tag_back
     for (int x=0; x<1; x++) {
//                                                                                                                          -            -         X         -
        if ((x%1000)==0) {
...
*/
        ...
        printf("2");
//        nbt_free_tag(tag_in);
        tag_back = read_nbt_from_memory((uint8_t*) location, length, NBT_PARSE_FLAG_USE_RAW);
        printf("3");
//        free(location);
//        location=NULL;
    }
        printf("4");
    write_nbt_file("r.0.0/Copy of 0.nbt", tag_back, NBT_WRITE_FLAG_USE_RAW);
        printf("5");
    nbt_free_tag(tag_in);
