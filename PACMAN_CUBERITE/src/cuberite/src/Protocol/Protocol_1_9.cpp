...
{
	ASSERT(m_State == 3);  // In game mode?
	double PosX = a_Painting.GetPosX()+chunk_offsetx*16;
	double PosY = a_Painting.GetPosY();
	double PosZ = a_Painting.GetPosZ()+chunk_offsetz*16;
...
void cProtocol_1_9_0::SendPickupSpawn(const cPickup & a_Pickup)
{
	...
	{  // TODO Use SendSpawnObject
		...
		Pkt.WriteBEDouble(a_Pickup.GetPosX()+chunk_offsetx*16);
		Pkt.WriteBEDouble(a_Pickup.GetPosY());
		Pkt.WriteBEDouble(a_Pickup.GetPosZ()+chunk_offsetz*16);
...
void cProtocol_1_9_0::SendPlayerMoveLook(void)
{
	...
	cPlayer * Player = m_Client->GetPlayer();
	Pkt.WriteBEDouble(Player->GetPosX()+chunk_offsetx*16);
	Pkt.WriteBEDouble(Player->GetPosY());
	Pkt.WriteBEDouble(Player->GetPosZ()+chunk_offsetz*16);
...
void cProtocol_1_9_0::SendExperienceOrb(const cExpOrb & a_ExpOrb)
{
	...
	Pkt.WriteBEDouble(a_ExpOrb.GetPosX()+chunk_offsetx*16);
	Pkt.WriteBEDouble(a_ExpOrb.GetPosY());
	Pkt.WriteBEDouble(a_ExpOrb.GetPosZ()+chunk_offsetz*16);
...
{
	ASSERT(m_State == 3);  // In game mode?
	double PosX = a_Entity.GetPosX()+chunk_offsetx*16;
	double PosZ = a_Entity.GetPosZ()+chunk_offsetz*16;
...
void cProtocol_1_9_0::SendTeleportEntity(const cEntity & a_Entity)
{
	...
	Pkt.WriteVarInt32(a_Entity.GetUniqueID());
	Pkt.WriteBEDouble(a_Entity.GetPosX()+chunk_offsetx*16);
	Pkt.WriteBEDouble(a_Entity.GetPosY());
	Pkt.WriteBEDouble(a_Entity.GetPosZ()+chunk_offsetz*16);
	Pkt.WriteByteAngle(a_Entity.GetYaw());
	Pkt.WriteByteAngle(a_Entity.GetPitch());
	Pkt.WriteBool(a_Entity.IsOnGround());
}
...
void cProtocol_1_9_0::SendUpdateBlockEntity(cBlockEntity & a_BlockEntity)
{
	...
	Pkt.WritePosition64(a_BlockEntity.GetPosX()+chunk_offsetx*16, a_BlockEntity.GetPosY(), a_BlockEntity.GetPosZ()+chunk_offsetz*16);
...
void cProtocol_1_9_0::WriteBlockEntity(cPacketizer & a_Pkt, const cBlockEntity & a_BlockEntity)
{
	...
	switch (a_BlockEntity.GetBlockType())
	{
		case E_BLOCK_BEACON:
		{
			...
			Writer.AddInt("x",         BeaconEntity.GetPosX()+chunk_offsetx*16);
			Writer.AddInt("y",         BeaconEntity.GetPosY());
			Writer.AddInt("z",         BeaconEntity.GetPosZ()+chunk_offsetz*16);
		...
		case E_BLOCK_COMMAND_BLOCK:
		{
			...
			Writer.AddInt("x", CommandBlockEntity.GetPosX()+chunk_offsetx*16);
			Writer.AddInt("y", CommandBlockEntity.GetPosY());
			Writer.AddInt("z", CommandBlockEntity.GetPosZ()+chunk_offsetz*16);
		...
		case E_BLOCK_HEAD:
		{
			...
			Writer.AddInt("x", MobHeadEntity.GetPosX()+chunk_offsetx*32);
			Writer.AddInt("y", MobHeadEntity.GetPosY());
			Writer.AddInt("z", MobHeadEntity.GetPosZ()+chunk_offsetz*32);
		...
		case E_BLOCK_FLOWER_POT:
		{
			...
			Writer.AddInt("x", FlowerPotEntity.GetPosX()+chunk_offsetx*16);
			Writer.AddInt("y", FlowerPotEntity.GetPosY());
			Writer.AddInt("z", FlowerPotEntity.GetPosZ()+chunk_offsetz*16);
		...
		case E_BLOCK_MOB_SPAWNER:
		{
			...
			Writer.AddInt("x", MobSpawnerEntity.GetPosX()+chunk_offsetx*16);
			Writer.AddInt("y", MobSpawnerEntity.GetPosY());
			Writer.AddInt("z", MobSpawnerEntity.GetPosZ()+chunk_offsetz*16);
