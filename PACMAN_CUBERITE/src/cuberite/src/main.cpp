...
static void WINAPI serviceMain_cuberite(DWORD argc, TCHAR *argv[])
//static void WINAPI serviceMain(DWORD argc, TCHAR *argv[])
{
	wchar_t applicationFilename[MAX_PATH];
	wchar_t applicationDirectory[MAX_PATH];

	GetModuleFileName(nullptr, (LPSTR)applicationFilename, sizeof(applicationFilename));  // This binary's file path.
	...
	SetCurrentDirectory((LPCSTR)applicationDirectory);

	g_StatusHandle = RegisterServiceCtrlHandler((LPCSTR)SERVICE_NAME, serviceCtrlHandler);
...
////////////////////////////////////////////////////////////////////////////////
// main:
int main_cuberite(int argc, char ** argv);
...
void SERVER_THREAD() {
    if (argc_main==0) {
        ...
        main_cuberite(glob_argc,glob_argv);
    } else {
        main_cuberite(argc_main,argv_main);
    }
//    main_cuberite(1,argv);
    printf("Server thread stopped\n");
    server_running=0;
}

sf::Thread SERVER(&SERVER_THREAD);


void launch_SERVER() {
    static char** argv = new char* [2];
    static char par0[100];
    static char par1[100];
    if (server_running==1) {
        printf("Server already running\n");
        return;
    }
    strcpy(par0,"program.exe");
    strcpy(par1,"");
    argv[0]=par0;
    argv[1]=par1;
    glob_argc=1;
...
int main_cuberite(int argc, char ** argv)
{
	...
	if (cRoot::m_RunAsService)
	{
			...
			{
				{ (LPSTR)SERVICE_NAME, (LPSERVICE_MAIN_FUNCTION)serviceMain_cuberite },
				{ nullptr, nullptr }
			};
	...
	cRoot::m_TerminateEventRaised=false;
    if (animator_running==0) {
        printf("Sending start animator message\n");
        send_message='4';
    }
	return EXIT_SUCCESS;
}
