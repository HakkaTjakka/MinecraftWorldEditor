...
vec4 nyan(vec2 p)
{
	...
	vec4 color = texture(texture_in,vec2(uv.x/3.0+210.0/256.0-nt+0.05,.5-uv.y-ny));
...

{
	...
	dir.y-=.02;
	float sunsize=7.-max(0.,texture(texture_in,vec2(.6,.2)).x)*5.; // responsive sun size
...
void main( )
{
	vec2 uv = (gl_FragCoord.xy-pos_correction.xy) / iResolution.xy*2.-1.;
	vec2 oriuv=uv;
