...
void main(  ){
    ...
    // Screen coordinates.
	vec2 uv = ((gl_FragCoord.xy-pos_correction) - iResolution.xy*.5)/iResolution.y;

    // Variable setup, plus rotation.
	float t = iTime/10.0, s, a, b, e;


    // Rotation the canvas back and forth.
    float th = sin(iTime*0.1)*sin(iTime*0.13)*4.;
    ...
    vec3 lp = vec3(cos(iTime)*0.375, sin(iTime)*0.1, -1.); // Light position - Back from the screen.
