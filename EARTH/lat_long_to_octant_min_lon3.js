...
//		console.log("-------------------------------------------------------------------------------------------------");
		...
		let out_index = {}
		let index=0;
		for (let lat_number=middle_lat; lat_number<from_lat+num_lats*size_lat; lat_number=lat_number+size_lat) {
...
//			lon = lon1+size_lon;
	...
	console.log("num_lats="+num_lats);
			...
			console.log("middle_lat="+middle_lat);
			console.log("middle_lon="+middle_lon);
...
//		for (let lat_number=middle_lat; lat_number<num_lats*size_lat; lat_number++) {

	        for (let lon_number=middle_lon; lon_number<from_lon+num_lons*size_lon; lon_number=lon_number+size_lon) {
...
//				console.log("lon="+lon);
                ...
                for (let octantLevel in foundOctants) {
                    ...
                    if ( octantLevel === '17' ) {
                        ...
                        ok=0;
                        let line_start= "["+lat_number.toString().padStart(3, ' ')+"]["+lon_number.toString().padStart(3, ' ')+"] (" + box.n.toFixed(13) + "," + box.w.toFixed(13) + "," + box.s.toFixed(13) + "," + box.e.toFixed(13) + ")";
...
//[#    0][  0/129][  0/ 79]=21427251635371524 lat=40.568390 lon=73.837738  fy=-0.107881 fx=-0.177676 f=0.207863
//[# 3079][112/129][ 23/ 79]=21426341726043542 lat=40.631561 lon=82.231293  fy=-0.044709 fx=0.129941 f=0.137417
//[# 4177][ 49/129][ 32/ 79]=21426251637061471 lat=40.656281 lon=85.222321  fy=-0.01999  fx=-0.043094 f=0.047505
//[# 4178][ 50/129][ 32/ 79]=21426251637061470 lat=40.656281 lon=85.225067  fy=-0.01999  fx=-0.040347 f=0.045028
//[# 4179][ 51/129][ 32/ 79]=21426251637061461 lat=40.656281 lon=85.227814  fy=-0.01999  fx=-0.037601 f=0.042584
//[# 4180][ 52/129][ 32/ 79]=21426251637061460 lat=40.656281 lon=85.230560  fy=-0.01999  fx=-0.034854 f= 0.04018
//[# 4181][ 53/129][ 32/ 79]=21426251637060571 lat=40.656281 lon=85.233307  fy=-0.01999  fx=-0.032108 f=0.037822
//[# 5201][ 41/129][ 40/ 79]=21426241637072471 lat=40.678253 lon=88.012848  fy=0.001983  fx=-0.065067 f=0.065097
//[# 5202][ 42/129][ 40/ 79]=21426241637072470 lat=40.678253 lon=88.015594  fy=0.001983  fx=-0.06232 f=0.062352
//[# 6372][ 51/129][ 49/ 79]=20537351627241427 lat=40.702972 lon=91.204376  fy=0.026702  fx=-0.037601 f=0.046117
//[# 6373][ 52/129][ 49/ 79]=20537351627241426 lat=40.702972 lon=91.207123  fy=0.026702  fx=-0.034854 f=0.043907
//[# 7602][120/129][ 58/ 79]=20537340736242740 lat=40.727692 lon=94.557953  fy=0.051421  fx=0.151913 f= 0.16038
//[# 7603][121/129][ 58/ 79]=20537340736242651 lat=40.727692 lon=94.560699  fy=0.051421  fx=0.15466 f=0.162984
//[# 9135][105/129][ 70/ 79]=20537241736272455 lat=40.760651 lon=98.735504  fy=0.08438   fx=0.110715 f=0.139204
//[# 9136][106/129][ 70/ 79]=20537241736272454 lat=40.760651 lon=98.738251  fy=0.08438   fx=0.113461 f=0.141398
//[# 9137][107/129][ 70/ 79]=20537241736272445 lat=40.760651 lon=98.740997  fy=0.08438   fx=0.116208 f=0.143612
//[# 9877][ 73/129][ 76/ 79]=20537240636376231 lat=40.777130 lon=100.756989 fy=0.10086   fx=0.022824 f= 0.10341
//[# 9878][ 74/129][ 76/ 79]=20537240636376230 lat=40.777130 lon=100.759735 fy=0.10086   fx=0.025571 f=0.104051
//						fy=(lat - (lat0+lat1)/2.0)*105;                                 
//						fx=lon - (lon1+lon0)/1.95+6.3//-6.15;                           
						...
						out[index]=f;                                                  
						out_x[index]=lon_number;                                       
						out_y[index]=lat_number;                                       
...
//				fx=middle_lons[index] - (lon1+lon0)/2.0;
...
//				fy=middle_lats[index] - (lat0+lat1)/2.0;
						...
						fx=middle_lons[index] -2.0*middle_lon + (lon1+lon0)/2.0;
						fy=middle_lats[index] +0.0*middle_lat - (lat0+lat1)/2.0;
